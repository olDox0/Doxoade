# doxoade/commands_test/test_vulcan_autopilot.py
import sys
...
def test_filter_candidates_skips_already_compiled_when_not_forced(monkeypatch):
    ...
    monkeypatch.setattr('doxoade.tools.vulcan.forge.assess_file_for_vulcan', lambda file_path: (True, None))
    result = ap._filter_candidates([{'file': 'a.py'}, {'file': 'b.py'}], force_recompile=False)
...
def test_filter_candidates_keeps_compiled_when_forced(monkeypatch):
    ...
    ap.advisor = _AdvisorStub({'b.py'})
    monkeypatch.setattr('doxoade.tools.vulcan.forge.assess_file_for_vulcan', lambda file_path: (True, None))
    result = ap._filter_candidates([{'file': 'a.py'}, {'file': 'b.py'}], force_recompile=True)
