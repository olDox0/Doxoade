# doxoade/commands_test/test_diagnose_git_context.py
...
def test_check_git_health_uses_target_file_parent_as_cwd(tmp_path):
    ...
    with patch('doxoade.diagnostic.inspector._run_git_command') as mock_git, patch('doxoade.diagnostic.inspector._get_last_commit_info', return_value=None), patch('doxoade.diagnostic.inspector._get_detailed_diff_stats', return_value=[]):
...
def test_get_detailed_diff_stats_handles_binary_numstat():
    with patch('doxoade.tools.git._run_git_command') as mock_git:
        mock_git.side_effect = ['-\t-\tassets/logo.png\n3\t1\tsrc/a.py', '']
