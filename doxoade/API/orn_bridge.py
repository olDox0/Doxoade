# doxoade/doxoade/API/orn_bridge.py
...
def _build_prompt(path: str, summary: dict[str, int], findings: list[dict[str, Any]]) -> str:
    ...
    for item in top_findings:
        ...
        err_match = re.search('(SyntaxError|TypeError|ValueError|NameError|ImportError|ModuleNotFoundError)', raw)
...
def _fallback_solution(findings: list[dict[str, Any]]) -> str:
    ...
    if 'unexpected indent' in msg.lower():
        return f'Sugestão: Veja {file_name}:{line}. Analise bem a indentação.{curr_hint}'
...
def _ensure_solution_text(text: str, findings: list[dict[str, Any]]) -> str:
    ...
    low = t.lower()
    generic = 'patch exato: syntaxerror' in low or ('causa raiz: syntaxerror' in low and len(t) < 120) or '```' in t or ('pass' in low and 'def ' in low)
    if generic:
        return _fallback_solution(findings)
...
def _run_orn_cli(command: list[str], timeout_s: int, workdir: str | None=None) -> tuple[bool, str]:
    try:
        proc = subprocess.run(command, capture_output=True, text=True, timeout=max(5, timeout_s), check=False, cwd=workdir)
...
def _command_from_path(path: Path) -> ResolvedOrnCommand | None:
    ...
    if p.is_dir():
        dir_candidates = [p / 'orn', p / 'orn.exe', p / 'orn.py', p / 'engine' / 'cli.py']
    ...
    if p.name.lower() == 'cli.py' and p.parent.name.lower() == 'engine':
        ...
        return ResolvedOrnCommand(command=[sys.executable, '-m', 'engine.cli'], workdir=str(project_root))
    if p.suffix.lower() == '.py':
        return ResolvedOrnCommand(command=[sys.executable, str(p)], workdir=_infer_workdir_for_path(p))
...
def _resolve_from_env_cmd() -> tuple[ResolvedOrnCommand | None, str]:
    ...
    if token_path:
        return (ResolvedOrnCommand(command=[*token_path.command, *parsed[1:]], workdir=token_path.workdir), 'env:ORN_CMD(cmd+path)')
...
def _resolve_from_common_paths() -> tuple[ResolvedOrnCommand | None, str]:
    ...
    rel_candidates = [Path('ORN') / 'orn', Path('ORN') / 'orn.exe', Path('ORN') / 'orn.py', Path('orn.py'), Path('engine') / 'cli.py', Path('ORN') / 'engine' / 'cli.py']
    for base in _candidate_bases():
        for rel in rel_candidates:
            ...
            if candidate.suffix.lower() == '.py':
                return (ResolvedOrnCommand(command=[sys.executable, str(candidate)], workdir=_infer_workdir_for_path(candidate)), f'scan:{candidate}')
...
def _register_orn_location(command: ResolvedOrnCommand, source: str) -> None:
    ...
    payload = {'updated_at_unix': int(time.time()), 'source': source, 'command': command.command, 'workdir': command.workdir}
...
def _persist_bridge_log(*, path: str, summary: dict[str, int], attempts: list[BridgeAttempt]) -> None:
    ...
    row = {'timestamp': int(time.time()), 'source': 'doxoade-check', 'path': path, 'summary': summary, 'attempts': [a.__dict__ for a in attempts]}
