...
class ResourceMonitor(threading.Thread):
    ...
    def __init__(self, pid):
        ...
        self.running = True
        self.daemon = True
        self.peaks = {'cpu_percent': 0.0, 'memory_mb': 0.0, 'io_read_start': 0, 'io_write_start': 0, 'io_read_max': 0, 'io_write_max': 0}
        ...
        try:
            ...
            while self.running:
                ...
                if cpu > self.peaks['cpu_percent']:
                    self.peaks['cpu_percent'] = cpu
                if mem > self.peaks['memory_mb']:
                    self.peaks['memory_mb'] = mem
                if r_tree > self.peaks['io_read_max']:
    ...
    def get_stats(self):
        ...
        return {'cpu': round(self.peaks['cpu_percent'], 1), 'ram': round(self.peaks['memory_mb'], 1), 'read': round(read_bytes / (1024 * 1024), 3), 'write': round(write_bytes / (1024 * 1024), 3)}
        ...
        while self.running:
            ...
            try:
                ...
                while frame:
                    ...
                    if any((x in norm_file for x in noise_check)) or norm_file.startswith(lib_path):
                        frame = frame.f_back
                        continue
...
class ChronosRecorder:
    ...
    def start_command(self, ctx):
        ...
        self.system_context = {'os': platform.system(), 'release': platform.release(), 'arch': platform.machine(), 'python': platform.python_version(), 'processor': platform.processor(), 'cores': psutil.cpu_count() if HAS_PSUTIL else 1}
