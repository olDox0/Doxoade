# doxoade/experiments/optimizer_systems/optimizer_benchmark.py
import time
...
class OptimizerBenchmark:
    ...
    def run(self, optimized_file: Path):
        ...
        return {
            "original_avg": sum(orig_times) / self.runs,
            "optimized_avg": sum(opt_times) / self.runs,
            "delta_pct": (
                (sum(orig_times) - sum(opt_times)) / sum(orig_times)
            ) * 100,
        }