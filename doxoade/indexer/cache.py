# doxoade/doxoade/indexer/cache.py
"""
...
class IndexCache:
    ...
    def is_valid(self, files: List[Path]) -> bool:
        ...
        try:
            for file_path in files:
                ...
                cursor.execute('SELECT checksum FROM file_metadata WHERE file_path = ?', (str(file_path),))
                row = cursor.fetchone()
    ...
    def load(self) -> Optional[Dict]:
        ...
        try:
            ...
            if cursor.fetchone()[0] == 0:
                return None
            index = {'functions': defaultdict(list), 'comments': {}, 'calls': defaultdict(set), 'docstrings': {}}
    ...
    def _load_functions(self, cursor, index: Dict) -> None:
        ...
        for name, file_path, line_num, docstring in cursor.fetchall():
            index['functions'][name].append({'file': file_path, 'line': line_num, 'name': name, 'docstring': docstring})
            if docstring:
