# doxoade/doxoade/commands/impact_systems/fluxogram_imports.py
"""
...
def to_mermaid(graph: 'FluxGraph', highlight_cycles: bool=True, target_module: str=None) -> str:
    ...
    def _short_name(mod: str) -> str:
        ...
        return mod
    lines = ['%%{init: {', "  'flowchart': {", "    'curve': 'basis',", "    'nodeSpacing': 80,", "    'rankSpacing': 150,", "    'padding': 20", '  }', '}}%%', 'flowchart TD']
...
def to_payload(graph: FluxGraph) -> Dict[str, Any]:
    ...
    return {'stats': graph_stats(graph), 'nodes': [{'id': mod, **attrs} for mod, attrs in sorted(graph.nodes.items())], 'edges': [{'source': edge.src, 'target': edge.dst, 'kind': edge.kind, 'imports': list(edge.imports)} for edge in graph.edges]}
...
def to_xml(graph: FluxGraph, *, include_cycles: bool=True) -> str:
    ...
    for edge in graph.edges:
        edge_el = ET.SubElement(edges_el, 'edge', source=edge.src, target=edge.dst, kind=edge.kind)
        if edge.imports:
