# doxoade/doxoade/commands/impact_systems/impact_html.py
import json
...
def to_html(mermaid_str: str, stats: dict, issues: list) -> str:
    ...
    final_html = template.replace('{{STYLE_CSS}}', style_css).replace('{{APP_JS}}', app_js).replace('{{GRAPH}}', mermaid_str).replace('{{DATA_JSON}}', data_json)
    return final_html