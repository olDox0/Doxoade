# -*- coding: utf-8 -*-
# doxoade/commands/impact_systems/impact_logic.py
import os
import ast
import click
...
def build_project_index(search_path: str, ignore_patterns: set, old_index: dict) -> dict:
    ...
    # 1. Varredura com filtro rigoroso (PASC 8.17)
    from ...dnm import DNM
    dnm = DNM(search_path)
    ...
    for fp in all_files:
        ...
        # 2. Check de Cache (Atena Strategy)
...
def get_external_consumers(state: ImpactState, func_filter: str = None) -> List[Dict]:
    ...
    for mod, data in state.index.items():
        ...
        if target in data.get("imports", []):
            # Verifica se o módulo chama funções do alvo
            hits = set(data.get("calls", [])).intersection(target_defines)
            if func_filter: hits = {h for h in hits if h == func_filter}
            if hits:
