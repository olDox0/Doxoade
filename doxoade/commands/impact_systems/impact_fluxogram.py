...
def build_import_fluxogram(
    ...
    *,
    ...
    # Foco opcional em um módulo específico
...
def _subgraph_from_target(graph: FluxGraph, target: str, depth: Optional[int] = 1) -> FluxGraph:
    ...
    if depth is None:
        # Expansão completa por conectividade
        while frontier:
...
def graph_stats(graph: FluxGraph) -> Dict[str, Any]:
    ...
    return {
        ...
        "incoming": incoming,
        "outgoing": outgoing,
        "isolated": sorted([m for m in graph.nodes if incoming.get(m, 0) == 0 and outgoing.get(m, 0) == 0]),
    }
