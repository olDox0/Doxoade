# doxoade/doxoade/commands/deepcheck.py
...
def _prepare_context(content):
    ...
    try:
        cc_map = {f.name: f.complexity for f in ComplexityVisitor.from_code(content).functions}
    except Exception:
        cc_map = {}
...
def _run_orchestrated_scan(file_path, func_filter, flags):
    ...
    if flags.get('git'):
        ...
        if git_raw:
            ...
            for node in [n for n in ast.walk(g_ctx['tree']) if isinstance(n, ast.FunctionDef)]:
                v = DeepAnalyzer(module_imports=g_ctx['imports'])
                v.visit(node)
    ...
    if func_filter:
        nodes = [n for n in nodes if n.name == func_filter]
    final_results = []
    for node in nodes:
        visitor = DeepAnalyzer(module_imports=ctx['imports'])
        visitor.visit(node)
        report = _render_deep_report(visitor, node.name, ctx['cc_map'].get(node.name, 1), as_json=flags.get('as_json'), show_vars=flags.get('variable'), show_flow=flags.get('flow'), compare_to=comp_map.get(node.name))
        if flags.get('flow') and (not flags.get('as_json')):
...
def deepcheck(file_path, func, variable, flow, as_json, compare_json, compare_git):
    ...
    try:
        ...
        if as_json:
            ...
            click.echo(Fore.YELLOW + 'Nenhuma função encontrada.')
        ...
        print(f'\x1b[31m[ ERROR ] {type(e).__name__} at L{line}: {e}\x1b[0m')