# -*- coding: utf-8 -*-
# doxoade/commands/run_systems/run_c_lang.py
...
C_SOURCE_EXTS: Final[dict[str, dict[str, object]]] = {
    ".c": {
        "compiler": "gcc",
        "std_flag": "-std=c11",
    },
...
def _resolve_toolchain(ext: str) -> tuple[str, str]:
    ...
    if not compiler_path:
        raise click.ClickException(
            f"Compilador '{compiler_name}' não encontrado no PATH. "
            f"Instale o w64devkit ou ajuste o PATH."
        )
...
def execute_binary(binary_path: str | os.PathLike[str]) -> None:
    ...
    if proc.returncode != 0:
        raise click.ClickException(
            f"Execução terminou com código {proc.returncode}: {binary.name}"
        )
...
#        click.echo("\033[33m[BUILD] Compilando...\033[0m")
#        exe_path = compile_c_family_source(script_path)
#    else:
#        click.echo("\033[32m[CACHE] Usando binário existente\033[0m")
    ...
    click.echo(f"\033[36m--- [RUN:C/C++] {exe_path.name} ---\033[0m")
    execute_binary(exe_path)
