...
def _format_git_status(status_line):
    ...
    file = parts[1]
    if code == 'A':
        return f'{Fore.GREEN}[NOVO] {file}{Style.RESET_ALL}'
    if code == 'M':
        return f'{Fore.YELLOW}[MODIFICADO] {file}{Style.RESET_ALL}'
    if code == 'D':
        return f'{Fore.RED}[DELETADO] {file} (PERIGO!){Style.RESET_ALL}'
    if code == 'R':
        return f'{Fore.CYAN}[RENOMEADO] {file}{Style.RESET_ALL}'
    return f'[{code}] {file}'
...
def _analyze_impact(remote, branch):
    ...
    incoming_changes = _run_git_command(['diff', '--name-status', f'HEAD..{remote_ref}'], capture_output=True)
    outgoing_changes = _run_git_command(['diff', '--name-status', f'{remote_ref}..HEAD'], capture_output=True)
    has_danger = False
