# doxoade/doxoade/commands/python.py
import sys
import os
import subprocess
import webbrowser
...
def _install_for_termux(logger):
    ...
    try:
        ...
        update_result = subprocess.run(['pkg', 'update', '-y'], capture_output=True, text=True, encoding='utf-8', errors='replace')
        ...
        install_result = subprocess.run(['pkg', 'install', 'python', '-y'], capture_output=True, text=True, check=True, encoding='utf-8', errors='replace')
