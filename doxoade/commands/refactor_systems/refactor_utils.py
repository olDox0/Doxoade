...
EXTRA_IGNORES = {'nppbackup', '.nppbackup', 'backup', 'temp', 'tmp', 'obj'}
ALL_SKIP = SYSTEM_IGNORES.union(EXTRA_IGNORES)
SKIP_DIRS = {
    '.git', '.hg', '.svn', '__pycache__', 'node_modules', '.mypy_cache', '.pytest_cache', '.ruff_cache',
    '.tox', '.venv', 'venv', 'env', 'build', 'dist'
...
class _FunctionCollector(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        self.hits.append(FunctionHit(name=node.name, file=self.file, line=getattr(node, 'lineno', 0), col=getattr(node, 'col_offset', 0), kind='def', qualname=self._qualname(node.name)))
        self.scope_stack.append(node.name)
        self.generic_visit(node)
        self.scope_stack.pop()

    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef) -> None:
        self.hits.append(FunctionHit(name=node.name, file=self.file, line=getattr(node, 'lineno', 0), col=getattr(node, 'col_offset', 0), kind='async def', qualname=self._qualname(node.name)))
        self.scope_stack.append(node.name)
        self.generic_visit(node)
        self.scope_stack.pop()
...
class _ReferenceCollector(ast.NodeVisitor):
    ...
    def _add_hit(self, node, name, context='usage'):
        self.hits.append(ReferenceHit(
            name=name,
            file=self.file,
    ...
    def visit_Name(self, node: ast.Name) -> None:
        if node.id in self.targets:
            self._add_hit(node, node.id)
        self.generic_visit(node)

    def visit_Attribute(self, node: ast.Attribute) -> None:
        # Captura Fore.RED quando pesquisamos por RED ou por Fore
        if node.attr in self.targets:
            self._add_hit(node, node.attr, context='attribute')
        self.generic_visit(node)
    ...
    def visit_Import(self, node: ast.Import) -> None:
        for alias in node.names:
            for t in self.targets:
                if t in alias.name:
                    # Reportamos 't' (o alvo), não 'alias.name' (o caminho completo)
                    self._add_hit(node, t, context='import')

    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        if node.module:
            for t in self.targets:
                if t in node.module:
                    self._add_hit(node, t, context='import_source')
        for alias in node.names:
            if alias.name in self.targets:
                self._add_hit(node, alias.name, context='import_item')
        self.generic_visit(node)

    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        if node.name in self.targets:
            self._add_hit(node, node.name, context='definition')
        self.generic_visit(node)
