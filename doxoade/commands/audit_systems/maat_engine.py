# doxoade/doxoade/commands/audit_systems/maat_engine.py
"""
...
class MaatEngine:
    ...
    def run_full_audit(self, target_files: list):
        ...
        for f in target_files:
            ...
            # FILTRO NEXUS: Ignora lixo, testes de regressão e diagnósticos
            if is_ignored(abs_f, self.root):
                continue
            
            # Filtro Adicional: Não julgar a própria infraestrutura de teste como produção
            if any(x in abs_f for x in ['regression_tests', 'diagnostic', 'fixtures', 'tests/']):
                continue
