# -*- coding: utf-8 -*-
# doxoade/commands/audit_systems/maat_weights.py
"""
...
class WeightGuard:
    ...
    def audit(self, files: list):
        ...
        for f in files:
            ...
            # 1. Checagem de Peso Físico
            size_kb = os.path.getsize(f) / 1024
            if size_kb > self.MAX_KB:
                findings.append({
                    'severity': 'ERROR', 'category': 'PASC-1.3',
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                for i, line in enumerate(f, 1):
                    # Se a linha contém o import mas tem # noqa, Ma'at perdoa.
                    if 'import sqlite3' in line: # noqa
                        if '# noqa' in line.lower():
                            continue
                            
                        if 'commands/' in file_path.replace('\\', '/'):
                            return {
                                'severity': 'ERROR', 'category': 'PASC-8.6',
                                'message': "Violação de Arquitetura: Comandos não podem importar sqlite3 diretamente. Use # noqa para ignorar.",
