# doxoade/doxoade/commands/guicheck.py
import os
import ast
import click
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            if node.func.attr == 'eval':
                logger.add_finding('WARNING', "Uso de 'eval' detectado.", file=file_path, line=node.lineno, details='O uso de eval() pode ser um risco de segurança se a entrada não for controlada.', snippet=_get_code_snippet(file_path, node.lineno))
...
def _perform_layout_analysis(layout_by_parent, file_path, logger):
    ...
    for parent, data in layout_by_parent.items():
        if len(data['methods']) > 1:
            line = min(data['lines'])
            logger.add_finding('ERROR', f"Uso misto de .pack() e .grid() no mesmo container '{parent}'.", file=file_path, line=line, details='Tkinter não permite usar .pack() e .grid() no mesmo widget pai.', snippet=_get_code_snippet(file_path, line))
