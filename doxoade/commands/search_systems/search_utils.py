# doxoade/doxoade/commands/search_systems/search_utils.py
...
def get_code_from_commit(commit_hash: str, query: str) -> list:
    ...
    import subprocess
    cmd = ['git', 'grep', '-n', '-i', '-e', query, commit_hash, '--', '*.py', '*.md', ':(exclude).doxoade/*']
    try:
        res = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', shell=False)
        if res.returncode != 0:
            return []
        ...
        for line in res.stdout.splitlines():
            parts = line.split(':', 3)
            if len(parts) >= 4:
                matches.append({'file': parts[1], 'line': parts[2], 'text': parts[3].strip()})
        return matches
    except Exception:
        return []
