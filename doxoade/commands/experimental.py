# doxoade/doxoade/commands/experimental.py
"""
...
def list_experiments() -> None:
    if not EXPERIMENTS_DIR.exists():
        ...
        return
    experiments = [d.name for d in EXPERIMENTS_DIR.iterdir() if d.is_dir() and (not d.name.startswith('_'))]
    if not experiments:
...
def run_experiment(name: str, argv: list[str]) -> int:
    ...
    spec = importlib.util.spec_from_file_location(f'doxoade.experiments.{name}.main', main_file)
