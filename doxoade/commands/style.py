# doxoade/doxoade/commands/style.py
"""
...
from doxoade.tools.filesystem import collect_files_to_analyze
...
def _execute_probe(probe_path: str, files: list, comment: bool) -> list:
    ...
    """
    payload = {'files': [os.path.abspath(f) for f in files], 'comments_only': comment}
    result = subprocess.run([sys.executable, probe_path], input=json.dumps(payload), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=60, shell=False)
...
def style(ctx, path: str, comment: bool, ignore: tuple):
    ...
    with ExecutionLogger('style', root_path, ctx.params) as logger:
        ...
        files = [path] if target_is_file else collect_files_to_analyze(_get_project_config(logger, start_path=path), ignore)
        if not files:
