# doxoade/doxoade/commands/test_mapper.py
import os
import ast
import click
...
class TestMapper:
    ...
    def __init__(self, root_path):
        ...
        self.map = {'covered': {}, 'orphans': [], 'loose_tests': []}
    ...
    def scan(self):
        ...
        tests = []
        for p in all_py:
            if self._is_ignored_source(p) and 'tests' not in p.parts:
                continue
            is_valid_test = True
    ...
    def _find_targets_in_test(self, test_path):
        ...
        try:
            with open(test_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            import re
            matches = re.findall('#\\s*TEST-TARGET:\\s*(.+)', content)
            for m in matches:
