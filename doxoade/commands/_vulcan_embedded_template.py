...
"""
Template do vulcan_embedded.py — Chronos Lite v4.
...
def _dump_vulcan_telemetry():
    ...
    # FIX HADES: Localiza o banco data/doxoade.db relativo à raiz do projeto
    
    _root = _find_project_root(script_file)
    try:
        from doxoade.database import DB_FILE
        db_path = Path(DB_FILE)
    except (ImportError, AttributeError):
        # Fallback caso o ambiente nao tenha o doxoade instalado
        db_path = Path.home() / '.doxoade' / 'doxoade.db'
#    db_path = Path(_root) / 'data' / 'doxoade.db' if _root else Path(sys.modules['doxoade'].__file__).parent / 'data' / 'doxoade.db'
    
#    if root:
#        local_cand = Path(root) / 'data' / 'doxoade.db'
