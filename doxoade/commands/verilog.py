# doxoade/doxoade/commands/verilog.py
import click
import os
import subprocess
import re
...
def parse_iverilog_output(output):
    ...
    findings = []
    pattern = re.compile('^(?P<file>.+?):(?P<line>\\d+):\\s*(?P<msg>.+)$')
    for line in output.splitlines():
        ...
        if match:
            ...
            if 'syntax error' in msg_lower:
                ...
                category = 'DUPLICATE'
                severity = 'CRITICAL'
            findings.append({'file': match.group('file'), 'line': int(match.group('line')), 'message': msg, 'severity': severity, 'category': category})
    return findings
...
def verilog(ctx, path, entrypoint, no_libs):
    ...
    with ExecutionLogger('verilog', path, ctx.params) as logger:
        ...
        if not check_iverilog_installed():
            click.echo(Fore.RED + "[ERRO] 'iverilog' não encontrado no PATH.")
            return
        ...
        if entrypoint:
            if os.path.exists(entrypoint):
                ...
                click.echo(Fore.RED + f"[ERRO] Arquivo '{entrypoint}' não encontrado.")
                return
        else:
            target_files = scan_verilog_files(path, ignore_list)
            click.echo(f'   > Verificando {len(target_files)} arquivos...')
        if not target_files:
            click.echo(Fore.YELLOW + 'Nenhum arquivo Verilog encontrado.')
            return
        files_with_errors = 0
            ...
            try:
                ...
                if result.returncode != 0 or 'error' in output.lower() or 'warning' in output.lower():
                    ...
                    if findings:
                        ...
                        for f in findings:
                            if not entrypoint and f['category'] == 'LINKING':
                                pass
