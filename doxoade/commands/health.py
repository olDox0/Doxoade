# doxoade/doxoade/commands/health.py
import sys
import subprocess
import os
import json
...
def health(ctx, path, ignore, output_format, complexity_threshold, min_coverage):
    ...
    with ExecutionLogger('health', path, arguments) as logger:
        if output_format == 'text':
            click.echo(Fore.CYAN + '[HEALTH] Iniciando check-up do projeto...')
                ...
                try:
                    ...
                    for func in visitor.functions:
                        if func.complexity > threshold:
                            findings.append({'severity': 'WARNING', 'message': f"Função '{func.name}' muito complexa ({func.complexity}).", 'details': f'Limite é {threshold}. Considere refatorar.', 'file': file_path, 'line': func.lineno})
                except Exception:
                    pass
    return findings
        ...
        for i, line in enumerate(lines):
            ...
            if any((pkg in line.lower() for pkg in CRITICAL_PACKAGES)) and '==' not in line:
                ...
                if pkg_name in CRITICAL_PACKAGES:
                    findings.append({'severity': 'WARNING', 'message': f"Pacote crítico '{pkg_name}' sem versão fixada (==).", 'details': 'Projetos estáveis devem pinar versões de bibliotecas principais.', 'file': 'requirements.txt', 'line': i + 1})
    except IOError:
        pass
    return findings
