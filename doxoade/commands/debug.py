...
@click.option('--memory',     '-m',  is_flag=True, help='Autópsia profunda de Memória (GC + Tracebacks).') # <-- NOVA FLAG
...
def debug(script, args, watch, bottleneck, no_compress, profile, memory, threshold):
    ...
    execute_debug(script, watch, bottleneck, threshold, no_compress, args, profile=profile, memory=memory) # <-- Passe memory adiante