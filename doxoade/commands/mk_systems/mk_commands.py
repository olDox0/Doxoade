...
def register_mk_options(f):
    ...
    f = click.option('--architecture', '-a', type=click.Path(exists=True), help='Cria estrutura baseada em arquivo de arquitetura.')(f)
    f = click.option('--learning', '-l', type=click.Path(exists=True), help='Cria estrutura baseada em arquivo de aprendizado (Gênese).')(f)
    return f
