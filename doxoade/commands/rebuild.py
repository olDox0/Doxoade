# doxoade/doxoade/commands/rebuild.py
"""
...
def _install_requirements_safe(path: str, logger: ExecutionLogger) -> bool:
    ...
    try:
        ...
        subprocess.run([venv_python, '-m', 'pip', 'install', '--upgrade', 'pip'], check=True, capture_output=True, cwd=path)
        click.echo(Fore.WHITE + '   > Sincronizando dependências (isso pode demorar)...')
        subprocess.run([venv_python, '-m', 'pip', 'install', '-r', req_file], check=True, capture_output=True, cwd=path)
        click.echo(Fore.WHITE + '   > Validando consistência do ambiente...')
        check = subprocess.run([venv_python, '-m', 'pip', 'check'], capture_output=True, text=True, cwd=path)
    ...
    with ExecutionLogger('rebuild', path, ctx.params) as logger:
        ...
        if not force:
            ...
            if not click.confirm(Fore.YELLOW + 'Deseja continuar?'):
                click.echo('Operação abortada.')
                return
        steps = [(Fore.YELLOW + '\n1. Limpeza', lambda: _remove_venv_resilient(path, logger)), (Fore.YELLOW + '2. Inicialização', lambda: _create_venv_safe(path, logger)), (Fore.YELLOW + '3. Sincronização', lambda: _install_requirements_safe(path, logger))]
