# doxoade/doxoade/commands/pedia_systems/pedia_io.py
"""
...
class KnowledgeBaseIO:
    ...
    def _locate_user_docs(self) -> Path:
        candidates = [self.user_root / 'docs', self.user_root / 'documentation', self.user_root / 'src' / 'docs']
        for c in candidates:
                ...
                for key, val in data.items():
                    ...
                    if isinstance(val, dict):
                        ...
                        if raw_content is None:
                            clean_val = val.copy()
                            clean_val.pop('title', None)
                            clean_val.pop('date', None)
    ...
    def _parse_markdown(self, path: Path, source: str, category: str) -> Article:
        ...
        for line in lines[:5]:
            if line.startswith('# '):
                title = line[2:].strip()
                break
        date_match = re.search('(?:Data|Date|Atualizado):\\s*(\\d{4}-\\d{2}-\\d{2})', content)
