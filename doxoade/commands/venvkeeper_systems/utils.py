# doxoade/doxoade/commands/venvkeeper_systems/utils.py
"""
...
def _build_log_query(search_term, command, file, category, severity, after, before, limit):
    ...
    clauses, params = ([], [])
    if search_term:
        clauses.append('f.message LIKE ?')
        params.append(f'%{search_term}%')
    if command:
        clauses.append('e.command LIKE ?')
...
def show_trace(ctx, filepath):
    ...
    if not target_file:
        ...
        if not target_file:
            click.echo(Fore.RED + "Nenhum arquivo de trace ('doxoade_trace_*.jsonl') encontrado no diretório atual.")
            return
    ...
    try:
        with open(target_file, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        if not lines:
            click.echo(Fore.YELLOW + 'O arquivo de trace está vazio.')
            return
...
def _get_github_repo_info():
    try:
        ...
        match = re.search('github\\.com[:/]([\\w-]+)/([\\w-]+)', url)
        if match:
        ...
        print(f'\x1b[1;34m[ FORENSIC ]\x1b[0m \x1b[1mFile: {f_name} | L: {line_n} | Func: _get_github_repo_info\x1b[0m')
        print(f'\x1b[31m  ■ Type: {type(e).__name__} | Value: {e}\x1b[0m')
    return 'unknown/unknown'

def _analyze_traceback(stderr_output):
    diagnostics = {'ModuleNotFoundError': 'Erro de importação. Lib não instalada no venv.', 'AttributeError': 'Erro de atributo. Cheque digitação ou versão da API.', 'FileNotFoundError': 'Arquivo não encontrado. Verifique caminhos e escapes.'}
    click.echo(Fore.YELLOW + '\n--- [DIAGNÓSTICO] ---')
    for key, message in diagnostics.items():
        if key in stderr_output:
            click.echo(Fore.CYAN + f'[PADRÃO DETECTADO] {message}')
            return
