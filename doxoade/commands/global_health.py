...
PROTECTED_PACKAGES = {"doxoade", "orn"}  # nunca remover
...
def _report_global_health(logger):
    ...
    if not duplicates:
        ...
        for name, installs in duplicates:
            ...
            logger.add_finding(
                "warning",
                f"Distribuição duplicada: {name}",
                category="GLOBAL-DUPLICATE",
                details="; ".join(f"{i['version']} @ {i['root']}" for i in installs),
            )
...
#    pip_root = Path(pip_path).resolve()
#    pip_root_parent = pip_root.parent.parent.parent if len(pip_root.parents) >= 3 else pip_root.parent
#
#    if os.path.normcase(str(pip_root_parent)) != os.path.normcase(str(Path(sys.executable).resolve().parent.parent)):
#        all_ok = False
#        msg = "O pip em uso não parece pertencer ao Python ativo."
#        click.echo(Fore.RED + f"   > [FALHA] {msg}")
#        logger.add_finding("error", msg, category="PIP-HEALTH", details=pip_path)
#    else:
#        click.echo(Fore.GREEN + "   > [OK] O pip pertence ao Python ativo.")
...
def _clean_pip_duplicates(logger):
    ...
    # Remove scripts pip fantasma fora do diretório ativo
    ...
    # Remove dist-info/egg-info duplicado do pip fora do site-packages ativo
...
def _clear_global_garbage(logger):
    ...
    for root in roots:
        ...
        for entry in root.iterdir():
            if entry.name in SAFE_GARBAGE_DIRS:
                ...
                # remove metadata órfão: se não houver pacote correspondente, pode sair
                base = entry.name
                norm = _normalize_name(base.split("-")[0])
                if norm in {_normalize_name(p) for p in PROTECTED_PACKAGES}:
                    continue

                # heurística simples: dist-info sem distribuição associada naquele root
                has_dist = any(
                    os.path.normcase(str(inst["root"])) == os.path.normcase(str(root))
                    for inst in by_name.get(norm, [])
                )
                if not has_dist:
                    removed_any |= _delete_path(entry, logger)
    ...
    for name, installs in by_name.items():
        ...
        for inst in installs:
            ...
            # remove apenas a pasta da distribuição/metadata quando possível
            # se não conseguir identificar, não remove nada agressivo
            dist = inst["dist"]
            try:
                files = list(dist.files or [])
            except Exception:
            ...
            # tenta localizar o .dist-info pela pasta da distribuição
            candidates = []
