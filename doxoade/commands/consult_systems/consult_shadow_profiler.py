# -*- coding: utf-8 -*-
# doxoade/commands/consult_systems/consult_shadow_profiler.py
"""
🦅 HORUS / NSR — Shadow Timeline Profiler v5.0 (Forensic Disk I/O & Memory Matrix).
Amostragem multi-thread de 200Hz com análise aprofundada de I/O de disco:
Throughput (KB/s), contagem de chamadas (Syscalls), picos e decomposição estrutural de RAM.
"""
import sys
import os
import time
...
class ShadowProfiler:
    """Profiler analítico com métricas forenses de I/O de disco, CPU e Composição de RAM."""

    def __init__(self, target_name: str, enabled: bool = False, sample_rate_hz: int = 200):
        self.target_name = target_name
        self.enabled = enabled
        self.interval = 1.0 / sample_rate_hz  # ~5ms (200Hz)
    ...
    def _sampler_loop(self):
        """Varredura de telemetria temporal em 200Hz com throughput de I/O por segundo."""
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        # Captura arquivos abertos pelo processo antes de fechar
        if self.proc:
            try:
    ...
    def _compile_full_matrix(self):
        """Compila métricas de hardware, I/O forense e decomposição de RAM."""
        ...
        # Métricas Finais de I/O Forense
        total_read_mb = 0.0
        total_write_mb = 0.0
        read_ops = 0
        write_ops = 0
    ...
    def _persist_matrix(self):
        """Exporta o dossiê NSR em JSON."""
        try:
            filename = f"timeline_{int(time.time())}_{self.target_name}.json"
            dump_path = self.output_dir / filename
            dump_path.write_text(json.dumps(self.report_data, indent=2), encoding="utf-8")
            self.report_data["dump_path"] = str(dump_path)
        except Exception:
            pass

    def render_hud(self, console: Optional[Console] = None):
        """Renderiza o HUD consolidado com Painel de I/O Forense e Decomposição de Recursos."""
        ...
        c.print(f"\n[bold magenta]🦅 SHADOW TIMELINE & FORENSIC MATRIX: {d['target'].upper()}[/bold magenta] "
                f"[dim]({wall:.1f}ms | {hw['num_cpus']} CPUs | {hw['total_samples']} Amostras a {hw['sample_rate_hz']}Hz)[/dim]")
        ...
        res_table.add_row(
            "CPU PROCESSO",
            cpu["ribbon"],
            f"Pico: [bold red]{cpu['peak_proc_cpu']}%[/bold red] de 1 Core | Méd: {cpu['avg_proc_cpu']}%"
        )
        res_table.add_row(
            "CPU TOTAL (PC)",
            _generate_shaded_ribbon(d["series"]["total_cpu_pct"]),
            f"Pico: [bold yellow]{cpu['peak_total_cpu']}%[/bold yellow] da máquina | Méd: {cpu['avg_total_cpu']}%"
        )
        res_table.add_row(
            "RAM RSS (TOTAL)",
            mem["ribbon_rss"],
            f"Início: {mem['boot_base_mb']}MB ➔ Pico: [bold green]{mem['peak_rss_mb']}MB[/bold green] (Δ {mem['delta_mb']:+.2f}MB)"
        )
        res_table.add_row(
            "HEAP COMANDO",
            mem["ribbon_heap"],
            f"Média: [bold yellow]{mem['avg_heap_mb']}MB[/bold yellow] | Pico: [bold yellow]{mem['peak_heap_mb']}MB[/bold yellow]"
        )
        res_table.add_row(
            "TAXA DISK READ",
            disk["ribbon_read_speed"],
            f"Pico: [bold cyan]{disk['peak_read_speed_kbps']} KB/s[/bold cyan] | Méd: {disk['avg_read_speed_kbps']} KB/s"
        )
        res_table.add_row(
            "TAXA DISK WRITE",
            disk["ribbon_write_speed"],
            f"Pico: [bold magenta]{disk['peak_write_speed_kbps']} KB/s[/bold magenta] | Méd: {disk['avg_write_speed_kbps']} KB/s"
        )
        ...
        io_files_str = "\n".join([f"    [dim]↳ {f}[/dim]" for f in disk["active_files"]]) if disk["active_files"] else "    [dim]Nenhum arquivo mantido aberto diretamente[/dim]"
        io_panel_content = (
            f"  [bold]Volume Total Transferido:[/bold]  Lidos: [bold cyan]{disk['total_read_mb']} MB[/bold cyan] | "
            f"Gravados: [bold magenta]{disk['total_write_mb']} MB[/bold magenta]\n"
            f"  [bold]Operações Físicas (Syscalls):[/bold] [bold cyan]{disk['read_ops']}[/bold cyan] Leituras | "
            f"[bold magenta]{disk['write_ops']}[/bold magenta] Gravações\n\n"
            f"  [bold yellow]📂 Alvos de I/O em Aberto Durante a Execução:[/bold yellow]\n{io_files_str}"
        )
        c.print(Panel(
            io_panel_content,
        ...
        # 3. Decomposição Analítica de Memória
        comp = mem["composition_pct"]
        comp_bar = _render_composition_bar(comp["boot"], comp["heap"], comp["native"], width=36)
        mem_panel_content = (
            f"  [bold]Distribuição Proporcional Média:[/bold]  {comp_bar}\n\n"
            f"  [bold blue]■ BOOT BASE (Doxoade Core):[/bold blue]  [white]{mem['boot_base_mb']} MB[/white] "
            f"[dim]({comp['boot']}%) — Carga inicial da VM, Vulcan e subsistemas[/dim]\n"
            f"  [bold yellow]■ HEAP DINÂMICO (Comando):[/bold yellow]   [white]{mem['avg_heap_mb']} MB[/white] "
            f"[dim]({comp['heap']}%) — Estruturas do algoritmo sob teste (pico: {mem['peak_heap_mb']}MB)[/dim]\n"
            f"  [bold cyan]■ CACHES & NATIVOS (C/OS):[/bold cyan]   [white]{mem['avg_native_mb']} MB[/white] "
            f"[dim]({comp['native']}%) — Páginas SQLite WAL, buffers C e I/O[/dim]"
        )
        c.print(Panel(
            mem_panel_content,
        ...
        # 5. Hotspots Triangulados
        paths = d.get("hot_paths", [])
        if paths:
