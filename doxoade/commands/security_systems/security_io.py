# doxoade/doxoade/commands/security_systems/security_io.py
"""
...
def get_progress_bar(items, label='Analisando'):
    return progressbar(items, label=Fore.WHITE + label, fill_char=Fore.CYAN + '█' + Fore.RESET, empty_char='░', show_pos=True, item_show_func=lambda x: f'({os.path.basename(x[0])}...)' if x and isinstance(x, list) else '')

def render_findings(findings: list, min_level: int, severity_map: dict):
    """Exibição coesa e informativa (MPoT-5.3)."""
    visible = [f for f in findings if isinstance(f, dict) and severity_map.get(f.get('severity', 'LOW').upper(), 1) >= min_level]
    if not visible:
