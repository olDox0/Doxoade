# doxoade/doxoade/commands/self_test.py
import os
import sys
import subprocess
import tempfile
...
TEST_CASES = {'check': {'filename': 'broken_code.py', 'content': 'def funcao_quebrada()\n    pass', 'expected_finding': 'invalid syntax'}, 'webcheck': {'filename': 'broken_page.html', 'content': '<a href="pagina_inexistente.html">Link Quebrado</a>', 'expected_finding': "Link quebrado para 'pagina_inexistente.html'"}}
...
def _run_doxoade_in_sandbox(command_to_test, sandbox_path):
    ...
    try:
        result = subprocess.run(command, cwd=sandbox_path, capture_output=True, text=True, encoding=sys.getdefaultencoding(), errors='backslashreplace')
