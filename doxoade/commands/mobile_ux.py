...
def show_file_info(file_path: Path):
    ...
    table.add_row('Modificado', str(datetime.fromtimestamp(stat.st_mtime)))
...
def show_extended_menu(ide_instance):
    ...
    while True:
        ...
        console.print(Panel('[bold cyan]🛠️  MENU AVANÇADO[/bold cyan]', style='bold white on blue'))
        options = [('1', '📊', 'Informações do arquivo'), ('2', '🔍', 'Buscar em arquivos'), ('3', '📋', 'Git status'), ('4', '📤', 'Compartilhar arquivo (Termux)'), ('5', '📋', 'Copiar caminho'), ('6', '🧹', 'Limpar cache Python'), ('7', '🔙', 'Voltar')]
...
def get_best_editor():
    ...
    for editor in editors:
        try:
            subprocess.run(['which', editor] if os.name != 'nt' else ['where', editor], capture_output=True, check=True)
            return editor
        except Exception:
            continue
    return 'nano'