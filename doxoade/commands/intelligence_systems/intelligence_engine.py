# doxoade/doxoade/commands/intelligence_systems/intelligence_engine.py
import os
import ast
...
try:
    ...
    from .intelligence_js import JSSemanticAnalyzer
except ImportError:
...
def analyze_file_chief(file_path: str, project_root: str, docs=False, source=False) -> dict:
    ...
    data = {'path': rel_path, 'size': os.path.getsize(file_path), 'god_assignment': 'Unknown'}
...
def _analyze_layer(level, b_path, curr_funcs):
    ...
    try:
        ...
        report = {'layer': level, 'date': datetime.fromtimestamp(mtime).isoformat(), 'lost_logic': []}
