# doxoade/doxoade/commands/apicheck.py
import ast
import os
import sys
import json
...
def _validate_call_against_contract(node, contract, file_path):
    ...
    for param in rules.get('required_params', []):
        if param not in provided_args:
            findings.append({'severity': 'ERROR', 'message': f"Chamada para '{contract.get('function')}' não possui o parâmetro obrigatório '{param}'.", 'details': f"Contrato '{contract.get('id')}' exige este parâmetro.", 'file': file_path, 'line': node.lineno})
    for param, bad_value in rules.get('forbidden_params', {}).items():
        for kw in node.keywords:
            if kw.arg == param and isinstance(kw.value, ast.Constant) and (kw.value.value == bad_value):
                findings.append({'severity': 'ERROR', 'message': f"Chamada para '{contract.get('function')}' usa o valor proibido '{param}={bad_value}'.", 'details': f"Contrato '{contract.get('id')}' proíbe este uso.", 'file': file_path, 'line': node.lineno})
    return findings
