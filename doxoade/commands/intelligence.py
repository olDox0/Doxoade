# doxoade/doxoade/commands/intelligence.py
import os
import json
import click
...
def recover(backup_path, output_path):
    ...
    if success:
        ...
        click.echo(f'\x1b[91m✘ {msg}\x1b[0m')
...
def _save_report(files, output, root, concat, focus, ai_export, console):
    ...
    if focus:
        ...
        economic_summary = {'total_files_scanned': len(files), 'total_files_in_report': len(report_files), 'god_distribution_in_report': _calculate_distribution(report_files), 'average_complexity_in_report': total_complexity / len(report_files) if report_files else 0, 'total_debt_tags_in_report': total_debt_tags, 'total_mpot_violations_in_report': total_mpot_violations}
        if focus == 'economic':
            summarized_report_files = []
            for f in report_files:
                summarized_report_files.append({'path': f.get('path'), 'god_assignment': f.get('god_assignment'), 'status': f.get('status'), 'complexity': f.get('complexity', 0), 'functions_count': len(f.get('functions', [])), 'classes_count': len(f.get('classes', [])), 'docstring_intent': f.get('docstring_intent', 'N/A'), 'debt_tags_count': len(f.get('debt_tags', [])), 'mpot_violations_count': f.get('mpot_4_violations', 0)})
        ...
        economic_summary = {'total_files_scanned': len(files), 'total_files_in_report': len(files), 'god_distribution_in_report': _calculate_distribution(files), 'average_complexity_in_report': total_complexity / len(files) if files else 0, 'total_debt_tags_in_report': total_debt_tags, 'total_mpot_violations_in_report': total_mpot_violations}
    report = {report_type: {'version': '2026.Chief.v2', 'generated_at': datetime.now(timezone.utc).isoformat(), 'target_project': os.path.basename(root), 'token_optimization': 'ENABLED' if concat else 'DISABLED', 'focus_applied': focus if focus else 'NONE'}, 'economic_summary': economic_summary, 'codebase_map': report_files}
    if ai_export:
