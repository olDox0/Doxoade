# doxoade/doxoade/commands/rescue_systems/scavenger_logic.py
"""
...
class Scavenger:
            ...
            for blob_hash in os.listdir(lost_dir):
                ...
                with open(path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if 'doxoade' in content or 'import click' in content:
                        recovered.append({'hash': blob_hash, 'preview': content[:100].replace('\n', ' '), 'full': content, 'date': datetime.datetime.fromtimestamp(os.path.getmtime(path))})
            return recovered
        ...
        for root, dirs, files in os.walk(self.root):
            if 'nppBackup' in root:
                for f in files:
                    ...
                    found.append({'name': f, 'path': full_p, 'date': datetime.datetime.fromtimestamp(os.path.getmtime(full_p))})
    ...
    def recover_from_npp_session(self):
        ...
        import os
        npp_path = os.path.expandvars('%APPDATA%\\Notepad++\\backup')
        if not os.path.exists(npp_path):
            return []
        backups = []
        for f in os.listdir(npp_path):
            full_p = os.path.join(npp_path, f)
            backups.append({'name': f, 'path': full_p, 'time': datetime.datetime.fromtimestamp(os.path.getmtime(full_p))})
