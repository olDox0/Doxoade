...
# ─── SYNTAX HIGHLIGHT DE LINHA ───────────────────────────────────────────────

# Padrões de keyword no início da linha (após indentação)
...
_RE_DECO    = re.compile(r'^(\s*)(@\w+)')
_RE_COMMENT = re.compile(r'^(\s*)(#.*)')

# Tokens inline que merecem destaque dentro de qualquer linha
_RE_SELF    = re.compile(r'\bself\b')
...
def _colorize_line(text: str) -> str:
    ...
    if m:
        ...
        return (
            f"{m.group(1)}"
            f"{Fore.YELLOW}{m.group(2)}{Style.RESET_ALL}"
            f"{Style.DIM}{rest}{Style.RESET_ALL}"
        )
    ...
    if m:
        ...
        return (
            f"{indent}"
            f"{Fore.CYAN}{async_kw}{def_kw}{Style.RESET_ALL}"
            f"{Style.BRIGHT}{Fore.GREEN}{fname}{Style.RESET_ALL}"
            f"{Style.DIM}{rest}{Style.RESET_ALL}"
        )
    ...
    if m:
        ...
        return (
            f"{indent}"
            f"{Fore.CYAN}{class_kw}{Style.RESET_ALL}"
            f"{Style.BRIGHT}{Fore.YELLOW}{cname}{Style.RESET_ALL}"
            f"{Style.DIM}{rest}{Style.RESET_ALL}"
        )
    ...
    if m:
        ...
        return (
            f"{indent}"
            f"{Fore.BLUE}{kw_part}{Style.RESET_ALL}"
            f"{Style.DIM}{rest}{Style.RESET_ALL}"
        )
    ...
    if m:
        ...
        return (
            f"{indent}"
            f"{Fore.MAGENTA}{kw}{Style.RESET_ALL}"
            f"{Style.DIM}{rest}{Style.RESET_ALL}"
        )

    # ── Nenhuma keyword especial: dim padrão ─────────────────────────────────
...
def _colorize_hot_line(text: str) -> str:
    ...
    """
    # Escapa o texto e aplica coloração inline básica
    out = text

    # Strings entre aspas simples ou duplas → verde
...
def render_hot_lines(line_data):
    ...
    for item in line_data[:6]:
        ...
        echo(
            f"       {Fore.YELLOW}{fname}:{lineno:<4}{Style.RESET_ALL} "
            f"({hits:>2} hits) > {Style.DIM}{content}{Style.RESET_ALL}"
        )
...
def render_lib_hot_lines(line_data):
    ...
    for item in line_data[:6]:
        ...
        echo(
            f"       {Fore.CYAN}{short}:{lineno:<4}{Style.RESET_ALL} "
            f"({hits:>2} hits) > {Style.DIM}{content}{Style.RESET_ALL}"
        )


# ─── CONTEXTO ANTECESSOR + POSTERIOR ─────────────────────────────────────────
...
def _read_context(abs_fname: str, lineno: int, before: int, after: int):
    ...
    for i in range(lineno + 1, lineno + after_count + 1):
        text = linecache.getline(abs_fname, i).rstrip()
        if text or not next_lines:          # inclui até a primeira linha real
            if text.strip():
...
def _render_predecessor_block(fname: str, lineno: int, hits: int,
                               layer: str, indent: str,
    ...
    prev_lines, hot_text, next_lines = _read_context(
        abs_f, lineno, context_before, context_after
    )

    # ── Tenta encontrar a função-pai no contexto anterior ────────────────────
    parent_fn = None
    ...
    echo(
        f"{indent}{Style.DIM}┌─ contexto · "
        f"{color}{short}:{lineno}{Style.RESET_ALL}"
        f"{parent_hint}"
    )

    # ── Linhas ANTES da hot-line ─────────────────────────────────────────────
    ...
    # ── Hot-line em destaque ─────────────────────────────────────────────────
    echo(
        f"{indent}{color}▶  {short}:{lineno}{Style.RESET_ALL}"
        f"  {Style.BRIGHT}{Fore.RED}◄ {hits:,} hits{Style.RESET_ALL}"
    )
    hot_colored = _colorize_hot_line(hot_text)
    echo(f"{indent}   {Style.BRIGHT}{color}{lineno:4} │{Style.RESET_ALL} {hot_colored}")

    # ── Linhas DEPOIS da hot-line ────────────────────────────────────────────
    if next_lines:
        echo(f"{indent}{Style.DIM}│{Style.RESET_ALL}")
...
def render_flow_map(flow_data: dict, io_read_mb: float = 0.0, io_write_mb: float = 0.0):
    ...
    if proj:
        ...
        for fname, stat in sorted(proj.items(), key=lambda x: x[1]['hits'], reverse=True)[:5]:
            ...
            echo(
                f"   {Style.DIM}│{Style.RESET_ALL}  {Fore.GREEN}{short:<20}{Style.RESET_ALL} "
                f"{bar} {stat['hits']:>5} hits  "
                f"{Style.DIM}score:{score:.0f}{Style.RESET_ALL}"
            )
    ...
    if proj and libs:
        ...
        if io_read_mb > 0 or io_write_mb > 0:
            io_label = (
                f"  {Style.DIM}(I/O  "
                f"{Fore.CYAN}R:{format_bytes(io_read_mb)}{Style.RESET_ALL}"
                f"{Style.DIM} / "
                f"{Fore.YELLOW}W:{format_bytes(io_write_mb)}{Style.RESET_ALL}"
    ...
    if libs:
        ...
        for fname, stat in sorted(libs.items(), key=lambda x: x[1]['hits'], reverse=True)[:5]:
            ...
            score = bottleneck_score(stat)
            echo(
                f"   {Style.DIM}│{Style.RESET_ALL}  {Fore.BLUE}{short:<20}{Style.RESET_ALL} "
                f"{bar} {stat['hits']:>5} hits  "
                f"{Style.DIM}score:{score:.0f}{Style.RESET_ALL}"
            )
        echo(f"   {Style.DIM}└{SEP}┘{Style.RESET_ALL}")
    ...
    if total_hits > 0:
        ...
        echo(
            f"\n   {Style.DIM}Distribuição de hits: "
            f"{Fore.GREEN}projeto {proj_pct:.0f}%{Style.RESET_ALL}"
            f"{Style.DIM} │ "
            f"{Fore.BLUE}libs {lib_pct:.0f}%{Style.RESET_ALL}"
        )
...
def render_critical_chain(chain: list, context_before: int = 3, context_after: int = 2):
    ...
    for step in chain:
        ...
        if layer != current_layer:
            ...
            echo(
                f"\n   {layer_color}{Style.BRIGHT}[{layer_label}]{Style.RESET_ALL}"
                f"{Style.DIM} {'─' * 40}{Style.RESET_ALL}"
            )

        _render_predecessor_block(
            fname, lineno, hits, layer,
            indent="     ",
...
def render_stats_table(stats):
    ...
    for cmd, data in sorted(stats.items(), key=lambda x: sum(x[1]['dur'])/len(x[1]['dur']), reverse=True):
        ...
        echo(
            f"{Fore.WHITE}{cmd:<15}{Style.RESET_ALL} | {len(data['dur']):<5} | "
            f"{avg(data['dur']):<10.0f} | {avg(data['ram']):<8.1f} | "
            f"{avg(data['io_r']):<8.2f} | {avg(data['io_w']):<8.2f}"
        )
...
def render_vulcan_stats(stats, verbose=False):
    ...
    for fn, data in sorted(stats.items(), key=lambda x: x[1].get('total_ms', 0), reverse=True)[:limit]:
        ...
        echo(
            f"     {Fore.CYAN}⬡{Style.RESET_ALL} {fn_disp:<38} │ "
            f"{hits:>4} hits | {fb_str} │ Avg: {avg:.3f}ms"
        )