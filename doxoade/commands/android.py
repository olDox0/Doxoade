# doxoade/doxoade/commands/android.py
import click
import os
import shutil
import sys
...
def get_android_download_dir():
    ...
    options = [os.path.expanduser('~/storage/downloads'), os.path.expanduser('~/storage/shared/Download'), '/sdcard/Download']
    for path in options:
...
def export_to_android(target, zip, name):
    ...
    with ExecutionLogger('android-export', target, {}) as _:
        if not ensure_storage_access():
            click.echo(Fore.RED + '[FALHA] Sem acesso ao armazenamento.')
            sys.exit(1)
        dest_root = get_android_download_dir()
        if not dest_root:
            click.echo(Fore.RED + '[FALHA] Pasta de Downloads não encontrada.')
            sys.exit(1)
        project_name = name or os.path.basename(os.path.abspath(target))
        export_folder = os.path.join(dest_root, 'DoxoadeExports')
        os.makedirs(export_folder, exist_ok=True)
        ignore_list = shutil.ignore_patterns('venv', '.git', '__pycache__', 'pytest_temp_dir', '.doxoade_cache', 'tmp', '*.pyc', '.pytest_cache')
        if zip:
...
def import_from_android(filename):
    ...
    if not src_root:
        return
    candidates = [os.path.join(src_root, 'DoxoadeExports', filename), os.path.join(src_root, filename)]
    src_path = None
    for c in candidates:
        if os.path.exists(c):
            src_path = c
