...
"""
import json
_LIB_MARKERS = ('site-packages/', 'site-packages\\', 'dist-packages/', 'dist-packages\\', 'venv/Lib/', 'venv\\Lib\\', 'venv/lib/', 'venv\\lib\\')
...
def parse_json_safe(data):
    ...
    try:
        ...
        print(f'\x1b[1;34m[ FORENSIC ]\x1b[0m \x1b[1mFile: {f_name} | L: {line_n} | Func: parse_json_safe\x1b[0m')
...
def aggregate_command_stats(rows):
    ...
    for row in rows:
        ...
        if row['duration_ms']:
            stats[cmd]['dur'].append(row['duration_ms'])
        if row['cpu_percent']:
            stats[cmd]['cpu'].append(row['cpu_percent'])
        if row['peak_memory_mb']:
            stats[cmd]['ram'].append(row['peak_memory_mb'])
...
def build_flow_data(proj_data: list, lib_data: list) -> dict:
    ...
    return {'proj': proj_stats, 'libs': lib_stats, 'total_hits': proj_hits + lib_hits, 'proj_hits': proj_hits, 'lib_hits': lib_hits}
...
def find_critical_chain(flow_data: dict, max_steps: int=4) -> list:
    ...
    for fname, stat in sorted(flow_data.get('proj', {}).items(), key=lambda x: x[1]['hits'], reverse=True)[:proj_limit]:
    ...
    for fname, stat in sorted(flow_data.get('libs', {}).items(), key=lambda x: x[1]['hits'], reverse=True)[:lib_limit]:
        for lineno, hits in stat['hot_lines'][:1]:
