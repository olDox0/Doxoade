# doxoade/doxoade/commands/termux_systems/termux_config.py
...
TERMUX_COLOR_BODY = '\n'.join(['background=#0a331a', 'foreground=#ffffff', 'cursor=#26bc5f', 'color2=#26bc5f'])
...
def apply_nanorc() -> None:
    path = HOME / '.nanorc'
    body = '\n'.join(['set linenumbers', 'set tabsize 4', 'set tabstospaces'])
    text = read_text(path)
...
def apply_micro_settings() -> None:
    ...
    settings.update({'colorscheme': 'default', 'syntax': True, 'cursorline': True, 'truecolor': 'auto', 'autoindent': False, 'smartpaste': False, 'softwrap': False, 'autoclose': False, 'wordwrap': False, 'smartindent': False, 'tabstospaces': True, 'tabsize': 4, 'diffgutter': True, 'savehistory': True, 'ruler': True, 'relativeruler': False, 'ftoptions': False})
...
def apply_micro_bindings() -> None:
    ...
    bindings.update({'Alt-s': 'HSplit', 'Alt-v': 'VSplit', 'Ctrl-w': 'NextSplit', 'Alt-w': 'NextSplit', 'Backtab': 'OutdentSelection|OutdentLine', 'Ctrl-z': 'Undo', 'Ctrl-y': 'Redo', 'Alt-d': 'command:diff'})
