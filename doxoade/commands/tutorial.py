# doxoade/doxoade/commands/tutorial.py
import click
import os
import shlex
import subprocess
    ...
    with ExecutionLogger('tutorial-interactive', '.', ctx.params):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                subprocess.run(['git', 'init', '--bare', fake_remote_path_str], capture_output=True, check=True)
                os.chdir(temp_dir)
                if not _prompt_and_run_sim_command("Primeiro, crie um projeto chamado 'meu-projeto-pratico'", 'doxoade init meu-projeto-pratico', runner_path):
                    return
                os.chdir(sim_project_path)
                if not _prompt_and_run_sim_command(f"Agora, publique no 'remote' falso com a mensagem 'Meu primeiro commit'.\nURL: {fake_remote_url}", f'doxoade git-new "Meu primeiro commit" "{fake_remote_url}"', runner_path):
                    return
...
def _prompt_and_run_sim_command(prompt, expected_command, runner_path):
    ...
    while True:
        ...
        if user_input.lower() in ['sair', 'exit']:
            click.echo(Fore.YELLOW + 'Simulação encerrada.')
            return False
        if user_input.lower() in ['ajuda', 'hint', 'help']:
            click.echo(Fore.YELLOW + f'O comando correto é: {expected_command}')
            continue
