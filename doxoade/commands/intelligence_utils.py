# doxoade/doxoade/commands/intelligence_utils.py
"""
...
def get_ignore_spec(root: str):
    ...
    import toml
    import pathspec
    default_patterns = ['.git/', '__pycache__/', 'venv/', '.venv/', '*.pyc', '.vscode/', '.idea/', 'dist/', 'build/', '*.bak', 'recovery_zone/', 'chief_dossier.json', 'node_modules/']
...
class ChiefInsightVisitor(ast.NodeVisitor):
    ...
    def __init__(self):
        self.stats = {'classes': [], 'functions': [], 'imports': {'stdlib': [], 'external': []}, 'complexities': [], 'mpot_4_violations': 0, 'docstrings': {}}
    ...
    def _analyze_func(self, node):
        ...
        self.stats['functions'].append({'name': node.name, 'lines': line_count, 'complexity': complexity, 'args': len(node.args.args), 'io_flow': self._detect_io_calls(node)})
    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if ext == '.json':
                ...
                headers = re.findall('^#+\\s+(.*)', content, re.MULTILINE)
...
def find_debt_tags(content: str) -> List[Dict[str, Any]]:
    debt = []
    patterns = '#\\s*(TODO|FIXME|BUG|HACK|ADTI)\\b[:\\s]*(.*)'
    for i, line in enumerate(content.splitlines(), 1):
        m = re.search(patterns, line, re.IGNORECASE)
        if m:
            debt.append({'line': i, 'tag': m.group(1).upper(), 'msg': m.group(2).strip()})
    return debt
...
def _print_forensic(func_name: str, e: Exception):
    ...
    print(f'\x1b[1;34m\n[ FORENSIC ]\x1b[0m \x1b[1mFile: {f_name} | L: {line_n} | Func: {func_name}\x1b[0m')
...
class SemanticAnalyzer:
    ...
    def __init__(self, code):
        ...
        try:
            ...
            print(f'\x1b[1;34m[ FORENSIC ]\x1b[0m \x1b[1mFile: {f_name} | L: {line_n} | Func: __init__\x1b[0m')
            print(f'\x1b[31m  ■ Type: {type(e).__name__} | Value: {e}\x1b[0m')
            self.tree = None

    def get_map(self):
        if not self.tree:
            return {'role': 'CORRUPT_SOURCE'}
        imports = [n.names[0].name for n in ast.walk(self.tree) if isinstance(n, ast.Import)]
        return {'role': self._infer_role(imports), 'classes': [n.name for n in ast.walk(self.tree) if isinstance(n, ast.ClassDef)], 'exported_symbols': [n.name for n in ast.walk(self.tree) if isinstance(n, ast.FunctionDef) if not n.name.startswith('_')], 'complexity_index': len([n for n in ast.walk(self.tree) if isinstance(n, (ast.If, ast.For, ast.While))])}

    def _infer_role(self, imports):
        if 'click' in imports:
            return 'ZEUS_CLI'
        if 'socket' in imports or 'requests' in imports:
            return 'POSEIDON_NET'
    ...
    def get_summary(self):
        ...
        return {'status': 'stable', 'classes': [n.name for n in ast.walk(self.tree) if isinstance(n, ast.ClassDef)], 'functions': [n.name for n in ast.walk(self.tree) if isinstance(n, ast.FunctionDef)], 'complexity': len([n for n in ast.walk(self.tree) if isinstance(n, (ast.If, ast.For, ast.While))])}
...
class NexusThothMapper:
    ...
    GOD_MAP = {'Zeus': {'keywords': ['cli', 'main', 'orchestrator'], 'imports': ['click', 'getopt.h', 'argparse.hpp']}, 'Poseidon': {'keywords': ['stream', 'socket', 'io', 'flow'], 'imports': ['asyncio', 'socket', 'sys/socket.h', 'curl/curl.h', 'arpa/inet.h']}, 'Hades': {'keywords': ['db', 'storage', 'cache'], 'imports': ['sqlite3', 'json', 'sqlite3.h', 'mysql.h', 'pqxx']}, 'Atena': {'keywords': ['logic', 'architecture', 'nexus'], 'imports': ['abc', 'memory', 'algorithm', 'vector']}, 'Anúbis': {'keywords': ['check', 'security', 'audit'], 'imports': ['hashlib', 're', 'openssl/sha.h', 'openssl/md5.h', 'regex.h']}}

    @classmethod
    def identify(cls, file_path, imports):
        name = file_path.lower()
        for god, criteria in cls.GOD_MAP.items():
            if any((k in name for k in criteria['keywords'])) or any((imp in imports for imp in criteria['imports'])):
                return god
        return 'Dionísio'
...
class CSemanticAnalyzer:
    ...
    def _parse(self):
        self.includes = re.findall('#include\\s*[<"]([^>"]+)[>"]', self.code)
        self.classes = re.findall('\\b(?:class|struct)\\s+([a-zA-Z_][a-zA-Z0-9_]*)\\b', self.code)
        func_pattern = '\\b[a-zA-Z_][a-zA-Z0-9_*:&<>\\s]+\\s+([a-zA-Z_][a-zA-Z0-9_]*)\\s*\\([^;]*\\)\\s*\\{'
        ...
        logic_branches = re.findall('\\b(?:if|for|while|catch|case)\\b', self.code)
        self.complexity = len(logic_branches) + 1

    def get_summary(self):
        """Retorna um mapa simplificado da 'alma' do arquivo C/C++."""
        return {'status': 'stable_c_cpp', 'classes': self.classes, 'functions': self.functions, 'complexity': self.complexity}

    def get_docstrings(self):
        """Extração simplificada de blocos de comentários."""
        docs = {}
        blocks = re.findall('/\\*\\*?(.*?)\\*/', self.code, re.DOTALL)
        if blocks:
...
class HTMLSemanticAnalyzer:
    ...
    def get_summary(self):
        ...
        return {'lines': self.lines_of_code, 'complexity': self.parser.complexity, 'html_stats': {'unique_tags': len(self.parser.tags), 'embedded_scripts': self.parser.scripts, 'embedded_styles': self.parser.styles, 'top_tags': top_tags}}
