...
def _verify_environment(target_path, logger):
    click.echo(Fore.WHITE + "Injetando sonda de verificação no ambiente do projeto...")
    requirements_file = os.path.join(target_path, 'requirements.txt')
    if not os.path.isfile(requirements_file):
        return {'status': 'ok', 'message': 'Nenhum requirements.txt para verificar.'}

    with open(requirements_file, 'r', encoding='utf-8') as f:
        packages_required = {line.strip().split('==')[0].lower().replace('_', '-') for line in f if line.strip() and not line.startswith('#')}
...
"""
    ...
    try:
        ...
        if not probe_data.get('is_isolated'):
            return {'status': 'error', 'message': 'Ambiente virtual não está devidamente isolado (contaminado)!' }
        
        installed_packages = set(probe_data.get('installed_packages', []))
        missing_packages = packages_required - installed_packages
        
        if missing_packages:
            return {'status': 'error', 'message': f"Pacotes listados em requirements.txt não foram encontrados no venv: {', '.join(sorted(list(missing_packages)))}"}

        return {'status': 'ok', 'message': 'Todas as dependências foram encontradas e o ambiente está isolado.'}
    except subprocess.CalledProcessError as e:
        details = f"STDOUT: {e.stdout}\nSTDERR: {e.stderr}"
        logger.add_finding('error', "A sonda de verificação falhou ao executar.", details=details)
        return {'status': 'error', 'message': f"A sonda de verificação falhou. Detalhes: {e.stderr}"}
    except Exception as e:
        logger.add_finding('error', "Falha desconhecida ao executar a sonda.", details=str(e))
        return {'status': 'error', 'message': f"Falha desconhecida ao executar a sonda: {e}"}
...
def _verify_and_guide_path(logger):
    ...
    if shutil.which(executable_name):
        ...
        # A lógica corrigida: encontramos a raiz real do projeto doxoade
        doxoade_root = _find_project_root()
        if not doxoade_root:
            click.echo(Fore.RED + "   > [ERRO CRÍTICO] Não foi possível localizar o diretório raiz da doxoade.")
            return False
        ...
        if os.name == 'nt':
            ...
            click.echo("2. FECHE E REABRA seu terminal.")
        else: # Para Linux, macOS, Termux, etc.
    ...
    with ExecutionLogger('doctor', path, arguments) as logger:
        ...
        try:
            ...
            if verification.get('status') == 'ok':
                click.echo(Fore.GREEN + f"   > [OK] {verification.get('message', 'Ambiente verificado.')}")
            else:
                click.echo(Fore.RED + f"   > [FALHA] {verification.get('message', 'Erro desconhecido na verificação.')}")
                logger.add_finding('warning', "Verificação inicial do ambiente falhou.", details=verification.get('message'))
                if click.confirm(Fore.CYAN + "\n     Deseja tentar reparar o ambiente?"):
                    ...
                    if post_repair_verification.get('status') == 'ok':
                        click.echo(Fore.GREEN + "   > [OK] Reparo bem-sucedido. O ambiente agora está saudável.")
                    else:
                        details = post_repair_verification.get('message', 'Erro desconhecido pós-reparo.')
