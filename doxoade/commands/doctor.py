# doxoade/doxoade/commands/doctor.py
"""
...
from doxoade.tools.pkg_manager import get_best_installer, run_install
...
def _install_dependencies(target_path: str, logger: ExecutionLogger) -> bool:
    ...
    if not venv_python:
        return False
    try:
        subprocess.run([venv_python, '-m', 'pip', 'install', '-r', reqs_file], check=True, capture_output=True)
...
def _verify_environment(target_path: str) -> Dict[str, str]:
    ...
    probe_script = "import sys, json, importlib.metadata; res = {'isolated': sys.prefix != sys.base_prefix, 'libs': [d.metadata['name'].lower() for d in importlib.metadata.distributions()]}; print(json.dumps(res))"
    try:
        result = subprocess.run([venv_python, '-c', probe_script], capture_output=True, text=True, check=True)
