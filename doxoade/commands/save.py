...
from .check import run_check_logic # Importa a lógica de análise diretamente

def _run_check_on_content(content, logger):
    ...
    try:
        ...
        results = run_check_logic(path=os.path.dirname(tmp_path), cmd_line_ignore=[], fix=False, debug=False, fast=True, no_imports=True, no_cache=True)
        os.remove(tmp_path)
        return results.get('findings', [])
    except Exception as e:
        logger.add_finding("WARNING", "Falha ao analisar conteúdo antigo para aprendizado.", details=str(e))
        return []
