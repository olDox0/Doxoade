# doxoade/doxoade/commands/debug_systems/debug_memory.py
"""
...
def get_memory_composition(limit: int=20) -> list:
    ...
    for obj_type, data in sorted_stats[:limit]:
        results.append({'type': obj_type, 'count': data['count'], 'size_kb': round(data['total_bytes'] / 1024, 2)})
    return results
...
def get_allocation_tracebacks(snapshot: tracemalloc.Snapshot, limit: int=5) -> list:
    ...
    for stat in top_stats[:limit]:
        ...
        for frame in stat.traceback:
            ...
            trace_chain.append({'file': fname, 'line': frame.lineno, 'code': code_line})
        if trace_chain:
            results.append({'size_kb': round(stat.size / 1024, 2), 'count': stat.count, 'traceback': trace_chain})
    return results