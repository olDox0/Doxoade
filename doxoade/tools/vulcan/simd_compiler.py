...
class SIMDContext:
    ...
    level_cap:   str              = "auto"    # 'sse2'|'avx'|'avx2'|'avx512f'|'native'|'auto'
    ...
    def effective_caps(self) -> SIMDCapabilities:
        ...
        # Recria caps zerando extensões acima do cap
        from dataclasses import replace
        kwargs = {}
        pairs = [
            ("sse2",    1), ("sse4_1", 2), ("sse4_2", 3),
            ("avx",     4), ("avx2",   5), ("avx512f", 6),
        ]
...
# ──────────────────────────────────────────────────────────────────────────────
# SIMDEnvironment — context manager que injeta CFLAGS
# ──────────────────────────────────────────────────────────────────────────────
...
class SIMDEnvironment:
    ...
    def __enter__(self) -> "SIMDEnvironment":
        ...
        for key in env_keys:
            ...
            # não duplicar flags já presentes
            new_val = (existing + " " + flags).strip()
            os.environ[key] = new_val

        # Marca para que o setup.py interno do Vulcan possa ler
...
# ──────────────────────────────────────────────────────────────────────────────
# SIMDPyxEnricher — transforma .pyx em SIMD-friendly
# ──────────────────────────────────────────────────────────────────────────────
...
class SIMDPyxEnricher:
    ...
    # Header injetado no topo do .pyx
    ...
    # Padrões de substituição para typed memoryviews
    _REPLACEMENTS = [
        # list → typed memoryview
        (r"\blist\b(\s+\w+)\b", r"double[::1]\1"),
        # np.ndarray sem tipo → typed
        (r"np\.ndarray\s+(\w+)", r"np.ndarray[np.float64_t, ndim=1] \1"),
        # range loop sobre len(arr) → paralelo candidato
        (r"for\s+(\w+)\s+in\s+range\s*\(\s*len\s*\((\w+)\)\s*\):",
         r"for \1 in range(len(\2)):  # SIMD-candidate"),
    ]
    ...
    def enrich(self, pyx_path: Path) -> Path:
        ...
        # Remove header antigo se existir (diretivas soltas)
        source = re.sub(r"^# cython:.*\n", "", source, flags=re.MULTILINE)

        # Injeta header
        flags_str = " ".join(self._caps.gcc_flags) if os.name != "nt" else " ".join(self._caps.msvc_flags)
        header = self._HEADER_TMPL.format(
            level=self._caps.best,
            flags=flags_str,
        )
        source = header + source

        # Aplica substituições conservadoras
        ...
        # Injeta `nogil` em funções cpdef que lidem com arrays numéricos
    ...
    def _inject_nogil_hints(self, source: str) -> str:
        ...
        for i, line in enumerate(lines):
            result.append(line)
            # Marca funções com arrays como candidatas a nogil manual
            if re.match(r"\s*(cpdef|cdef)\s+\w.*\(.*\[.*\].*\):", line):
...
# ──────────────────────────────────────────────────────────────────────────────
# SIMDForge — wrapper de alto nível
# ──────────────────────────────────────────────────────────────────────────────
...
class SIMDForge:
    ...
    def compile_module(
        ...
        return CompileResult(
            ...
            ok            = ok,
            error         = err,
            simd_level    = self.caps.best,
            flags_used    = flags,
            pyx_enriched  = enriched,
            bin_path      = bin_path,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                proc = subprocess.run(
                    ...
                    cwd=str(pyx_path.parent), env=env,
                )

                if proc.returncode != 0:
                    return False, (proc.stderr or proc.stdout)[-500:]

                # Copia .so/.pyd para bin_dir
    ...
    def compile_batch(
        self,
        targets: list[tuple[str, Path]],   # [(module_name, pyx_path), ...]
        bin_dir: Path | None = None,
        ...
        with SIMDEnvironment(self._ctx):
            with ThreadPoolExecutor(max_workers=workers) as pool:
                futures = {
                    pool.submit(self.compile_module, name, path, bin_dir): name
                ...
                for fut in as_completed(futures):
                    try:
                        ...
                        results.append(CompileResult(
                            module_name=name, ok=False, error=str(e)
                        ))

        return results

    # ── Integração com LibForge ────────────────────────────────────────────────
...
def estimate_gain(caps: SIMDCapabilities | None = None) -> str:
    ...
    caps = caps or detect_simd()
    gains = {
        "avx512f": "30–60% em operações vetorizadas (512-bit lanes)",
        "avx2":    "20–45% em operações vetorizadas (256-bit lanes)",
        "avx":     "15–30% em operações vetorizadas (256-bit float)",
        "sse4.2":  "10–20% em operações escalares e comparação de strings",
        "sse4.1":  "8–15% em operações de ponto flutuante",
        "sse2":    "5–10% vs código escalar base",
        "neon":    "20–40% em operações vetorizadas ARM (128-bit)",
        "none":    "0% — nenhuma extensão vetorial disponível",
    }
...
def get_simd_report(caps: SIMDCapabilities | None = None) -> dict:
    ...
    return {
        **caps.to_dict(),
        "estimated_gain": estimate_gain(caps),
        "cython_directives": caps.cython_directives,
        "define_macros": caps.define_macros,
    }