# -*- coding: utf-8 -*-
# doxoade/tools/vulcan/regression_registry.py
"""
RegressionRegistry
==================
...
MIN_SPEEDUP_DEFAULT = 1.10   # abaixo de 10% de ganho → regressão
...
class RegressionEntry:
    __slots__ = (
        "file_path", "func_name", "speedup",
        "attempts",  "status",    "last_updated",
    )
    ...
    def from_dict(cls, d: dict) -> "RegressionEntry":
        return cls(
            ...
            status       = d.get("status", STATUS_AGGRESSIVE),
            last_updated = d.get("last_updated", 0.0),
        )

    def __repr__(self) -> str:
        return (
            f"RegressionEntry({self.func_name!r}, "
            f"speedup={self.speedup:.2f}x, attempts={self.attempts}, "
...
class RegressionRegistry:
    ...
    def excluded_funcs_for_file(self, file_path: str) -> FrozenSet[str]:
        ...
        return frozenset(
            ...
            if k.startswith(f"{prefix}:") and e.status == STATUS_EXCLUDED
        )
    ...
    def aggressive_funcs_for_file(self, file_path: str) -> FrozenSet[str]:
        ...
        return frozenset(
            e.func_name
            for k, e in self._data.items()
            if k.startswith(f"{prefix}:") and e.status == STATUS_AGGRESSIVE
        )
    ...
    def report(self) -> dict:
        ...
        return {
            ...
            "excluded":         counts.get(STATUS_EXCLUDED, 0),
            "retry_aggressive": counts.get(STATUS_AGGRESSIVE, 0),
    ...
    def render_cli(self) -> None:
        ...
        print(f"  Total   : {r['total']} entrada(s)   "
              f"{R}Excluídas: {r['excluded']}{RST}   "
              f"{Y}Retry-Agressivo: {r['retry_aggressive']}{RST}")
        ...
        for e in r["entries"]:
            ...
            print(
                ...
                f"{color}{e['status']:<20}{RST} "
                f"{e['attempts']:>10}  {DIM}{fname}{RST}"
            )
        print()