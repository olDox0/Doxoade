# doxoade/doxoade/tools/vulcan/simd_detector.py
"""
──────────────────────────────────────────────────────────────────────────────
...
class SIMDCapabilities:
    ...
    def level(self) -> int:
        ...
        _map = {'avx512f': 6, 'avx2': 5, 'avx': 4, 'sse4.2': 3, 'sse4.1': 2, 'sse2': 1, 'neon': 1}
    ...
    def gcc_flags(self) -> list[str]:
        ...
        if self.avx512f:
            flags += ['-mavx512f', '-mavx512cd', '-mfma', '-mavx2']
        elif self.avx2:
            flags += ['-mavx2', '-mfma']
        elif self.avx:
            flags.append('-mavx')
        elif self.sse4_2:
            flags += ['-msse4.2', '-msse4.1']
        elif self.sse4_1:
            flags.append('-msse4.1')
        elif self.sse2:
            flags.append('-msse2')
        if self.fma and (not self.avx2):
            flags.append('-mfma')
        if self.bmi2:
            flags.append('-mbmi2')
        if self.bmi1 and (not self.bmi2):
            flags.append('-mbmi')
        if self.popcnt:
    ...
    def cython_directives(self) -> dict:
        ...
        """
        base = {'boundscheck': False, 'wraparound': False, 'cdivision': True, 'nonecheck': False, 'language_level': 3}
        if self.level >= 4:
    ...
    def define_macros(self) -> list[tuple[str, str | None]]:
        ...
        macros: list[tuple] = []
        if self.sse2:
            macros.append(('VULCAN_HAS_SSE2', None))
        if self.avx:
            macros.append(('VULCAN_HAS_AVX', None))
        if self.avx2:
            macros.append(('VULCAN_HAS_AVX2', None))
        if self.avx512f:
            macros.append(('VULCAN_HAS_AVX512F', None))
        if self.fma:
            macros.append(('VULCAN_HAS_FMA', None))
        if self.neon:
            macros.append(('VULCAN_HAS_NEON', None))
        return macros

    def to_dict(self) -> dict:
        return {'arch': self.arch, 'vendor': self.vendor, 'best': self.best, 'level': self.level, 'flags': {'sse2': self.sse2, 'sse4_1': self.sse4_1, 'sse4_2': self.sse4_2, 'avx': self.avx, 'avx2': self.avx2, 'avx512f': self.avx512f, 'fma': self.fma, 'bmi1': self.bmi1, 'bmi2': self.bmi2, 'neon': self.neon}, 'gcc_flags': self.gcc_flags, 'msvc_flags': self.msvc_flags}
...
def _detect_cpuinfo() -> SIMDCapabilities | None:
    ...
    try:
        ...
        return SIMDCapabilities(mmx='mmx' in flags_raw, sse='sse' in flags_raw, sse2='sse2' in flags_raw, sse3='sse3' in flags_raw, ssse3='ssse3' in flags_raw, sse4_1='sse4_1' in flags_raw, sse4_2='sse4_2' in flags_raw, avx='avx' in flags_raw, avx2='avx2' in flags_raw, avx512f='avx512f' in flags_raw, fma='fma' in flags_raw, bmi1='bmi1' in flags_raw, bmi2='bmi2' in flags_raw, popcnt='popcnt' in flags_raw, neon='neon' in flags_raw, arch=info.get('arch_string_raw', platform.machine()), vendor=info.get('vendor_id_raw', ''))
    except Exception:
        return None
...
def _detect_proc_cpuinfo() -> SIMDCapabilities | None:
    ...
    try:
        ...
        if not flags_set:
            return None
        return SIMDCapabilities(mmx='mmx' in flags_set, sse='sse' in flags_set, sse2='sse2' in flags_set, sse3='sse3' in flags_set or 'pni' in flags_set, ssse3='ssse3' in flags_set, sse4_1='sse4_1' in flags_set, sse4_2='sse4_2' in flags_set, avx='avx' in flags_set, avx2='avx2' in flags_set, avx512f='avx512f' in flags_set, fma='fma' in flags_set, bmi1='bmi1' in flags_set, bmi2='bmi2' in flags_set, popcnt='popcnt' in flags_set, neon='neon' in flags_set, arch=platform.machine(), vendor=vendor)
    except Exception:
        return None
...
def _detect_cpuid() -> SIMDCapabilities | None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            compile_args = [compiler, src, f'/Fe{exe}', '/nologo'] if os.name == 'nt' else [compiler, src, '-o', exe, '-O0']
        ...
        return SIMDCapabilities(sse2=flags.get('sse2', False), sse4_1=flags.get('sse4_1', False), sse4_2=flags.get('sse4_2', False), avx=flags.get('avx', False), avx2=flags.get('avx2', False), avx512f=flags.get('avx512f', False), fma=flags.get('fma', False), bmi1=flags.get('bmi1', False), bmi2=flags.get('bmi2', False), popcnt=flags.get('popcnt', False), arch=platform.machine())
    except Exception:
        return None
...
def _detect_windows_kernel32() -> SIMDCapabilities | None:
    ...
    try:
        ...
        return SIMDCapabilities(sse2=sse2, sse4_1=sse4_1, sse4_2=sse4_2, avx=avx or False, avx2=avx2 or False, avx512f=avx512f or False, fma=fma or False, bmi1=bmi1 or False, bmi2=bmi2 or False, popcnt=popcnt or sse4_2, arch=platform.machine(), vendor=vendor)
    except Exception:
        return None
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name, cmd_tmpl in compilers:
                cmd = [part.replace('{src}', src).replace('{exe}', exe) for part in cmd_tmpl]
                try:
                    r = subprocess.run(cmd, capture_output=True, timeout=20, cwd=tmp)
            ...
            if not flags:
                return (None,) * 7
            return (flags.get('avx', None), flags.get('avx2', None), flags.get('avx512f', None), flags.get('fma', None), flags.get('bmi1', None), flags.get('bmi2', None), flags.get('popcnt', None))
    except Exception:
        return (None,) * 7
...
def _cpuid_via_model_string() -> tuple:
    ...
    name = name.lower()
    import re
    intel_match = re.search('i[3579]-(\\d{4,5}[a-z]*)', name)
    if intel_match:
        model_str = intel_match.group(1)
        digits = re.match('(\\d+)', model_str).group(1)
        if len(digits) == 5:
            gen = int(digits[:2])
        else:
            gen = int(digits[0])
    ...
    if 'xeon' in name:
        if re.search('(platinum|gold|silver|bronze)\\s*\\d{4}', name):
            return (True, True, True, True, True, True, True)
        if re.search('e[57]-\\d{4}\\s*v[4-9]', name):
            return (True, True, False, True, True, True, True)
        if re.search('e[57]-\\d{4}\\s*v3', name):
            return (True, True, False, True, True, True, True)
        return (True, False, False, False, False, False, True)
    if 'ultra' in name and 'core' in name:
        return (True, True, False, True, True, True, True)
    if any((x in name for x in ('ryzen', 'threadripper', 'epyc'))):
        zen4 = bool(re.search('ryzen\\s*[579]\\s*[789]\\d{3}', name)) or 'zen 4' in name
        return (True, True, zen4, True, True, True, True)
    if re.search('amd\\s*fx', name):
        return (True, False, False, False, False, False, True)
...
def _detect_gcc_march_flags() -> dict | None:
    ...
    if not shutil.which('gcc'):
        return None
    try:
        r = subprocess.run(['gcc', '-march=native', '-Q', '--help=target'], capture_output=True, text=True, timeout=8)
        if r.returncode != 0:
            return None
...
def _detect_sysctl() -> SIMDCapabilities | None:
    ...
    try:
        ...
        machine = platform.machine().lower()
        if machine == 'arm64':
            return SIMDCapabilities(neon=True, arch='arm64', vendor='Apple')
        return SIMDCapabilities(sse2=sysctl_bool('hw.optional.sse2'), sse4_1=sysctl_bool('hw.optional.sse4_1'), sse4_2=sysctl_bool('hw.optional.sse4_2'), avx=sysctl_bool('hw.optional.avx1_0'), avx2=sysctl_bool('hw.optional.avx2_0'), avx512f=sysctl_bool('hw.optional.avx512f'), fma=sysctl_bool('hw.optional.fma'), arch=platform.machine(), vendor='Intel')
    except Exception:
        return None
...
def detect() -> SIMDCapabilities:
    ...
    """
    strategies = [('py-cpuinfo', _detect_cpuinfo), ('/proc/cpuinfo', _detect_proc_cpuinfo), ('kernel32', _detect_windows_kernel32), ('sysctl', _detect_sysctl), ('cpuid-probe', _detect_cpuid)]
...
def _refine_via_model_string(base: SIMDCapabilities) -> SIMDCapabilities:
    ...
    try:
        ...
        return replace(base, avx=avx or base.avx, avx2=avx2 or base.avx2, avx512f=avx512f or base.avx512f, fma=fma or base.fma, bmi1=bmi1 or base.bmi1, bmi2=bmi2 or base.bmi2, popcnt=popcnt or base.popcnt)
    except Exception:
        return base
