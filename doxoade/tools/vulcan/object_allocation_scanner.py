# doxoade/doxoade/tools/vulcan/object_allocation_scanner.py
"""
...
class ReduceStrategy(str, Enum):
    TYPED_MEMORYVIEW = 'typed_memoryview'
    CDEF_STRUCT = 'cdef_struct'
    CDEF_VAR = 'cdef_var'
    PREALLOCATE = 'preallocate'
    INDEX_LOOP = 'index_loop'
    BYTEARRAY_BUFFER = 'bytearray_buffer'
    INLINE_CDEF = 'inline_cdef'
    NUMPY_ARRAY = 'numpy_array'
    EXPLICIT_PARAMS = 'explicit_params'
    CONDITIONAL_FLOW = 'conditional_flow'
    YIELD_STREAM = 'yield_stream'
...
class ModuleAllocReport:
    ...
    def hot_functions(self) -> list[FunctionAllocReport]:
        ...
        return sorted([f for f in self.functions if f.total_score > 3], key=lambda f: f.total_score, reverse=True)
...
class ObjectAllocationScanner(ast.NodeVisitor):
    ...
    def _report(self) -> FunctionAllocReport:
        ...
        if key not in self._func_reports:
            self._func_reports[key] = FunctionAllocReport(func_name=key, lineno=self._current_func_lineno)
        return self._func_reports[key]

    def _add(self, node: ast.AST, pattern: AllocPattern, strategy: ReduceStrategy, cost: int=1, confidence: str='medium', auto_fix: bool=False):
        self._report().sites.append(AllocationSite(pattern=pattern, strategy=strategy, lineno=getattr(node, 'lineno', 0), col=getattr(node, 'col_offset', 0), func_name=self._current_func, snippet=self._snippet(node), in_loop=self._in_loop(), cost=cost, confidence=confidence, auto_fix=auto_fix))
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        args = node.args
        if args.kwarg:
            self._add(node, AllocPattern.KWARGS_DICT, ReduceStrategy.EXPLICIT_PARAMS, cost=2, confidence='medium')
        self.generic_visit(node)
    ...
    def visit_BinOp(self, node: ast.BinOp):
        if isinstance(node.op, ast.Add) and isinstance(node.left, ast.List) and isinstance(node.right, ast.List) and self._in_loop():
            self._add(node, AllocPattern.LIST_CONCAT, ReduceStrategy.PREALLOCATE, cost=2, confidence='high', auto_fix=True)
        self.generic_visit(node)
    _NUMERIC_VAR_RE = re.compile('^(?:score|total|count|num|n|idx|size|len|sum|acc|result|alloc|cost|hits|weight|rate|delta|offset|pos|step|total_\\w+|\\w+_count|\\w+_n|\\w+_score|\\w+_total|[ijk]|[ijk]\\d*)$', re.IGNORECASE)
    ...
    def visit_AugAssign(self, node: ast.AugAssign):
        ...
        if isinstance(node.value, ast.Constant) and (not isinstance(node.value.value, str)):
            self.generic_visit(node)
            return
        is_str_literal = isinstance(node.value, ast.Constant) and isinstance(node.value.value, str)
        is_fstring = isinstance(node.value, ast.JoinedStr)
        if is_str_literal or is_fstring:
            self._add(node, AllocPattern.STR_CONCAT, ReduceStrategy.BYTEARRAY_BUFFER, cost=2, confidence='high', auto_fix=False)
        elif isinstance(node.value, ast.Name):
            rhs_name = node.value.id
            if self._NUMERIC_VAR_RE.match(rhs_name):
                self.generic_visit(node)
                return
            self._add(node, AllocPattern.STR_CONCAT, ReduceStrategy.BYTEARRAY_BUFFER, cost=2, confidence='low', auto_fix=False)
        self.generic_visit(node)
    ...
    def visit_Call(self, node: ast.Call):
        if self._in_loop():
            ...
            if isinstance(func, ast.Name) and func.id in ('int', 'float', 'bool', 'complex'):
                self._add(node, AllocPattern.BOXED_NUMERIC, ReduceStrategy.CDEF_VAR, cost=1, confidence='high', auto_fix=True)
            elif isinstance(func, ast.Name) and func.id == 'dict':
                self._add(node, AllocPattern.DICT_TEMP, ReduceStrategy.CDEF_STRUCT, cost=3, confidence='medium')
            elif isinstance(func, ast.Name) and func.id == 'enumerate' and node.args and isinstance(node.args[0], ast.Call) and isinstance(node.args[0].func, ast.Name) and (node.args[0].func.id == 'range'):
                self._add(node, AllocPattern.RANGE_ENUMERATE, ReduceStrategy.INDEX_LOOP, cost=1, confidence='high', auto_fix=True)
            elif isinstance(func, ast.Name) and func.id == 'zip':
                self._add(node, AllocPattern.ZIP_TEMP, ReduceStrategy.INDEX_LOOP, cost=1, confidence='high', auto_fix=True)
        self.generic_visit(node)

    def visit_Lambda(self, node: ast.Lambda):
        if self._in_loop():
            self._add(node, AllocPattern.LAMBDA_HOTPATH, ReduceStrategy.INLINE_CDEF, cost=2, confidence='medium')
        self.generic_visit(node)
        ...
        if self._in_loop():
            if isinstance(node.value, ast.Subscript):
                ...
                if isinstance(sl, ast.Slice):
                    ...
                    is_str_method_result = isinstance(value_node, ast.Call) and isinstance(value_node.func, ast.Attribute) and (value_node.func.attr in ('hexdigest', 'digest', 'decode', 'encode', 'strip', 'lstrip', 'rstrip', 'lower', 'upper', 'replace', 'split', 'join', 'format'))
                    is_attr_access = isinstance(value_node, ast.Attribute) and value_node.attr in ('name', 'stem', 'suffix', 'parent')
                    has_step = sl.step is not None
                    if not (is_str_method_result or is_attr_access or has_step):
                        self._add(node, AllocPattern.SLICE_COPY, ReduceStrategy.TYPED_MEMORYVIEW, cost=2, confidence='high', auto_fix=True)
        self.generic_visit(node)

    def visit_Return(self, node: ast.Return):
        if isinstance(node.value, ast.ListComp):
            self._add(node, AllocPattern.LIST_COMP_RETURN, ReduceStrategy.YIELD_STREAM, cost=3, confidence='medium', auto_fix=False)
        self.generic_visit(node)
    ...
    def visit_Try(self, node: ast.Try):
        if self._in_loop() and node.handlers:
            for handler in node.handlers:
                if len(node.body) == 1:
                    self._add(node, AllocPattern.EXCEPTION_FLOW, ReduceStrategy.CONDITIONAL_FLOW, cost=3, confidence='medium')
                    break
        self.generic_visit(node)
        ...
        if self._in_loop():
            for elt in node.elts:
                if isinstance(elt, ast.List):
                    self._add(node, AllocPattern.NESTED_LIST_FLAT, ReduceStrategy.NUMPY_ARRAY, cost=len(node.elts), confidence='medium')
                    break
        self.generic_visit(node)
...
def scan_pyx(pyx_source: str, path: Path | None=None) -> ModuleAllocReport:
    ...
    """
    cleaned = '\n'.join((line if not line.lstrip().startswith(('cdef ', 'cpdef ', 'ctypedef ')) else 'pass  # ' + line.strip() for line in pyx_source.splitlines()))
    return scan_source(cleaned, path)
_STRATEGY_LABELS = {ReduceStrategy.TYPED_MEMORYVIEW: 'typed memoryview (double[::1])', ReduceStrategy.CDEF_STRUCT: 'cdef struct', ReduceStrategy.CDEF_VAR: 'cdef tipado (elimina boxing)', ReduceStrategy.PREALLOCATE: 'array pré-alocado', ReduceStrategy.INDEX_LOOP: 'loop por índice (elimina zip/enumerate)', ReduceStrategy.BYTEARRAY_BUFFER: 'bytearray buffer + join', ReduceStrategy.INLINE_CDEF: 'cdef inline (elimina lambda)', ReduceStrategy.NUMPY_ARRAY: 'np.ndarray tipado', ReduceStrategy.EXPLICIT_PARAMS: 'parâmetros explícitos (elimina **kwargs)', ReduceStrategy.CONDITIONAL_FLOW: 'if/else (elimina try/except como fluxo)', ReduceStrategy.YIELD_STREAM: 'yield / buffer pré-alocado'}
_PATTERN_LABELS = {AllocPattern.LIST_CONCAT: 'list concat em loop', AllocPattern.TUPLE_UNPACK: 'tuple unpack em loop', AllocPattern.DICT_TEMP: 'dict temporário em loop', AllocPattern.LIST_COMP_RETURN: 'list comprehension retornada', AllocPattern.STR_CONCAT: 'str += em loop', AllocPattern.BOXED_NUMERIC: 'boxing numérico em loop', AllocPattern.RANGE_ENUMERATE: 'enumerate(range()) em loop', AllocPattern.ZIP_TEMP: 'zip() em loop', AllocPattern.SLICE_COPY: 'slice copy em loop', AllocPattern.EXCEPTION_FLOW: 'try/except como controle de fluxo', AllocPattern.KWARGS_DICT: '**kwargs em hot path', AllocPattern.LAMBDA_HOTPATH: 'lambda em loop', AllocPattern.NESTED_LIST_FLAT: 'lista aninhada em loop'}
...
def render_report(report: ModuleAllocReport, verbose: bool=False) -> None:
    ...
    print(f'  Funções     : {len(report.functions)} analisadas, {len(report.hot_functions)} com alocações críticas')
    ...
    for func in report.hot_functions:
        ...
        print(f'  {color}{BOLD}{func.func_name}{RESET}  (linha {func.lineno})  score={func.total_score}  ~{func.estimated_allocs_per_call} allocs/call')
        for site in sorted(func.sites, key=lambda s: s.score, reverse=True):
            ...
            print(f'    {conf_color}●{RESET} L{site.lineno:<4} {_PATTERN_LABELS.get(site.pattern, site.pattern.value):<38}{in_loop_tag}{fix_tag}')
            if verbose:
