...
class VulcanAutopilot:
    ...
    def scan_and_optimize(self, candidates=None, force_recompile=False, max_workers: int | None=None, use_pitstop: bool=True, streaming: bool=True):
        ...
        if auto_mode and candidates:
            ...
            if auto_trimmed:
                print(f'   {Fore.BLUE}↷ Modo automático: limitando lote para {len(candidates)} alvos (restantes: {auto_trimmed}){Fore.RESET}')
        if not candidates:
    ...
    def _run_pitstop(self, candidates: list[dict], force_recompile: bool, max_workers: int, streaming: bool) -> None:
        ...
        try:
            if streaming and len(candidates) > _STREAMING_THRESHOLD:
                print(f'   {Fore.CYAN}   > Modo streaming ativado (forge ∥ compile){Fore.RESET}')
                stats = engine.run_streaming(candidates=candidates, max_workers=max_workers, force_recompile=force_recompile, on_result=_on_result)
            else:
                stats = engine.run(candidates=candidates, max_workers=max_workers, force_recompile=force_recompile, on_result=_on_result)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                for result in executor.map(self._process_target, candidates):
                    try:
                        ...
                        print(f'\x1b[31m ■ {fname}:{lineno} {e}\x1b[0m')
            if success_count > 0:
    ...
    def _process_target(self, candidate: dict) -> dict:
        ...
        result = _forge_worker({'file_path': file_path, 'foundry': str(self.env.foundry), 'bin_dir': str(self.env.bin_dir), 'pid_registry': self._pid_registry, 'prevalidated': bool(candidate.get('__vulcan_validated')), 'project_root': str(self.root)})
