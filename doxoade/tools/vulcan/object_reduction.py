# doxoade/doxoade/tools/vulcan/object_reduction.py
"""
...
class TransformResult:
    ...
    def summary(self) -> str:
        ...
        return f'{len(self.changes)} transformação(ões), ~{self.allocs_removed} alloc(s) eliminada(s){bak}'
...
class _TextTransformer:
    ...
    def _fix_boxed_numeric(self, site: AllocationSite):
        ...
        original = line
        new = re.sub('\\bint\\(([^)]+)\\)', '<int>(\\1)', line)
        new = re.sub('\\bfloat\\(([^)]+)\\)', '<double>(\\1)', new)
        new = re.sub('\\bbool\\(([^)]+)\\)', '<bint>(\\1)', new)
        if new != original:
            self._set_line(site.lineno, new)
            self.changes.append(f'L{site.lineno}: boxing numérico → cast C  ({original.strip()[:50]})')
            self.allocs_removed += site.cost

    def _fix_enumerate_range(self, site: AllocationSite):
        line = self._line(site.lineno)
        original = line
        m = re.match('^(\\s*)for\\s+(\\w+)\\s*,\\s*(\\w+)\\s+in\\s+enumerate\\(range\\(([^)]+)\\)\\)\\s*:', line)
        if m:
            indent, idx_var, val_var, n = m.groups()
            new = f'{indent}for {idx_var} in range({n}):\n'
            self._set_line(site.lineno, new)
            self.changes.append(f'L{site.lineno}: enumerate(range({n})) → range({n})')
            self.allocs_removed += site.cost
            return
        m2 = re.match('^(\\s*)for\\s+(\\w+)\\s*,\\s*(\\w+)\\s+in\\s+enumerate\\(([^)]+)\\)\\s*:', line)
        if m2:
            indent, idx_var, val_var, seq = m2.groups()
            new = f'{indent}__{seq.strip()}_len = len({seq.strip()})\n{indent}for {idx_var} in range(__{seq.strip()}_len):  # OBJ-REDUCE: enumerate → index\n{indent}    {val_var} = {seq.strip()}[{idx_var}]\n'
            self._set_line(site.lineno, f'{indent}for {idx_var} in range(len({seq.strip()})):  # OBJ-REDUCE\n')
            self.changes.append(f'L{site.lineno}: enumerate({seq.strip()}) → range+index')
            self.allocs_removed += site.cost

    def _fix_zip(self, site: AllocationSite):
        line = self._line(site.lineno)
        original = line
        m = re.match('^(\\s*)for\\s+(.+?)\\s+in\\s+zip\\(([^,]+),\\s*([^)]+)\\)\\s*:', line)
        if m:
            ...
            if len(vars_list) == 2:
                ...
                new = f'{indent}for _zi in range(min(len({sa}), len({sb}))):  # OBJ-REDUCE: zip → index\n{indent}    {va} = {sa}[_zi]\n{indent}    {vb} = {sb}[_zi]\n'
                self._set_line(site.lineno, new)
                self.changes.append(f'L{site.lineno}: zip({sa}, {sb}) → index loop')
                self.allocs_removed += site.cost

    def _fix_str_concat(self, site: AllocationSite):
        line = self._line(site.lineno)
        m = re.match('^(\\s*)(\\w+)\\s*\\+=\\s*(.+)', line.rstrip('\n'))
        if not m:
            return
        indent, var, rhs = m.groups()
        rhs_stripped = rhs.strip()
        is_str_rhs = rhs_stripped.startswith(('"', "'", 'f"', "f'", 'b"', "b'")) or rhs_stripped.startswith('"""') or rhs_stripped.startswith("'''")
        if not is_str_rhs:
            return
        new = f'{indent}_buf_{var}.append({rhs_stripped})  # OBJ-REDUCE: str→buffer\n'
        self._set_line(site.lineno, new)
        self.changes.append(f'L{site.lineno}: {var} += ... → buffer.append (requer init do buffer)')
        self.allocs_removed += site.cost
...
class _CythonDirectiveInjector:
    ...
    """
    _INT_NAMES = re.compile('\\b(i|j|k|n|m|idx|count|size|length|pos|offset|step|start|end|total)\\b')
    _FLOAT_NAMES = re.compile('\\b(x|y|z|val|value|result|acc|score|weight|prob|rate|ratio|delta|eps)\\b')
    ...
    def inject(self) -> tuple[str, list[str]]:
        ...
        while i < len(lines):
            line = lines[i]
            m = re.match('^((?:cpdef|cdef|def)\\s+\\w[\\w\\s]*\\s+)?def\\s+(\\w+)\\s*\\(', line)
            if not m:
                m = re.match('^(cpdef|cdef)\\s+\\w+\\s+(\\w+)\\s*\\(', line)
            if m:
                func_name = m.group(2) if m.lastindex >= 2 else None
                func_report = next((f for f in self._report.functions if f.func_name == func_name), None)
                result_lines.append(line)
                i += 1
                if func_report and func_report.sites:
                    injected = self._build_cdef_block(func_report, lines, i)
                    if injected:
                        result_lines, i = self._insert_after_docstring(result_lines, lines, i, injected)
                        changes.append(f'{func_name}: injetadas {len(injected)} variável(is) cdef')
                continue
            result_lines.append(line)
            i += 1
        return ('\n'.join(result_lines), changes)

    def _build_cdef_block(self, func_report, all_lines: list[str], body_start: int) -> list[str]:
        """Constrói bloco de `cdef` para as variáveis detectadas na função."""
        body_src = '\n'.join(all_lines[body_start:body_start + 80])
        cdef_vars: list[str] = []
        seen: set[str] = set()
        for m in re.finditer('\\bfor\\s+(\\w+)\\s+in\\s+range\\(', body_src):
            var = m.group(1)
            if var not in seen:
                cdef_vars.append(f'        cdef Py_ssize_t {var}')
                seen.add(var)
        for m in re.finditer('\\b(\\w+)\\s*[+\\-\\*\\/]?=\\s*[\\d\\.\\(\\-]', body_src):
            var = m.group(1)
        ...
        for site in func_report.sites:
            if site.pattern != AllocPattern.BOXED_NUMERIC:
                continue
            m = re.search('\\b(int|float|double)\\((\\w+)\\)', site.snippet)
            if m:
                typ, var = m.groups()
                if var not in seen:
    ...
    def _insert_after_docstring(self, result_lines: list[str], all_lines: list[str], i: int, cdef_block: list[str]) -> tuple[list[str], int]:
        """
...
def reduce_file(path: Path, level: int=2, dry_run: bool=False) -> TransformResult:
    """
    ...
    result = TransformResult(source_path=path, original_source=source, transformed=transformed, changes=changes, allocs_removed=allocs, level=level)
    if not dry_run and result.has_changes:
        result.backup_path = _backup_file(path)
        path.write_text(transformed, encoding='utf-8')
    return result

def reduce_source(source: str, path: Path | None=None, level: int=2, is_pyx: bool=False) -> TransformResult:
    ...
    if level >= 2 and is_pyx:
        ...
        changes.extend(cdef_changes)
        allocs += len(cdef_changes) * 2
    return TransformResult(source_path=path, original_source=source, transformed=transformed, changes=changes, allocs_removed=allocs, level=level)
