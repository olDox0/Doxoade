# doxoade/doxoade/tools/vulcan/lib_forge.py
"""
...
class LibForge:
    ...
    """
    _NATIVE_HEAVY_LIBS = frozenset({'numpy', 'pandas', 'scipy', 'torch', 'tensorflow', 'cv2', 'opencv-python', 'matplotlib', 'pygame', 'kivy', 'pyqt5', 'pyside2', 'pyside6', 'grpcio', 'cryptography', 'pillow', 'pil', 'lxml', 'sqlalchemy'})
    ...
    def compile_library(self, lib_name: str, run_optimizer: bool=True, simd_ctx=None) -> tuple[bool, str]:
        """
        Localiza, copia, otimiza e compila funções elegíveis da biblioteca.
        """
        if lib_name.lower() in self._NATIVE_HEAVY_LIBS:
            return (False, f"[EVASÃO DE RISCO] A biblioteca '{lib_name}' possui núcleo nativo (C/C++).\n   O Vulcan bloqueou a forja desta biblioteca para evitar degradação\n   de performance causada pelo overhead da C-API (Paradoxo Cython).")
        with tempfile.TemporaryDirectory(prefix=f'vulcan_lib_{lib_name}_') as temp_dir:
            ...
            opt_stats: dict = {'files_processed': 0, 'files_optimized': 0, 'files_skipped': 0, 'bytes_saved': 0, 'docstrings_removed': 0, 'dead_branches': 0, 'imports_removed': 0, 'locals_minified': 0}
            if run_optimizer:
            ...
            if not modules_generated:
                if errors:
                    return (False, f'Compilação falhou em todos os módulos.\nErros ({len(errors)}):\n' + '\n'.join(errors[:3]))
                return (False, f"Nenhuma função elegível encontrada em '{lib_name}' ({functions_skipped} descartadas por I/O ou estado mutável).")
            ...
            summary = f"Biblioteca '{lib_name}' forjada com sucesso!\n   Binários gerados : {len(moved)}\n   Funções nativas  : {functions_compiled}\n   Falhas no link   : {len(failed)}\n   Fontes otimizados: {opt_stats.get('files_optimized', 0)}/{opt_stats.get('files_processed', 0)} arquivos ({bytes_saved:,} bytes economizados){simd_line}"
            return (True, summary)
    ...
    def _find_in_venv(self, lib_name: str) -> Path | None:
        ...
        for d in reversed(site_dirs):
            if d not in sys.path:
                sys.path.insert(0, d)
        try:
            variants_to_try = [lib_name, lib_name.replace('-', '_'), lib_name.replace('_', '-')]
            ...
            sys.path = old_path
        base = lib_name.lower()
        name_variants = {lib_name, base, base.replace('-', '_'), base.replace('_', '-'), lib_name.replace('-', '_'), lib_name.replace('_', '-')}
        for sp in site_dirs:
            sp_path = Path(sp)
    ...
    def _optimize_sources(work_copy: Path) -> dict:
        try:
            ...
            return {'files_processed': 0, 'files_optimized': 0, 'files_skipped': 0, 'bytes_saved': 0, 'docstrings_removed': 0, 'dead_branches': 0, 'imports_removed': 0, 'locals_minified': 0}
