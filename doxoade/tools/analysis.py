# doxoade/doxoade/tools/analysis.py
import ast
import hashlib
import re
import json
...
def _find_returns_and_risks_in_function(func_node):
    ...
    for node in ast.walk(func_node):
        if isinstance(node, ast.Return) and node.value:
            ...
            risks.append({'lineno': node.lineno, 'message': 'Acesso a dicionário/lista sem tratamento.', 'details': f"Acesso direto a '{ast.unparse(node)}' pode causar 'KeyError' ou 'IndexError'."})
...
def _analyze_function_flow(tree, content):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            ...
            dossiers.append({'name': node.name, 'lineno': node.lineno, 'params': params, 'returns': returns, 'risks': risks, 'complexity': complexity, 'complexity_rank': _get_complexity_rank(complexity)})
    return dossiers
