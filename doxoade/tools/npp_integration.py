# doxoade/doxoade/tools/npp_integration.py
import json
import os
import time
...
def signal_notepadpp(file_path: str, findings: list, project_root: str):
    ...
    payload = {'protocol': 'Doxoade-NPP-v1', 'source': os.path.abspath(file_path).replace('\\', '/'), 'timestamp': time.time(), 'findings': [{'line': f.get('line', 0), 'msg': f.get('message', 'No message'), 'sev': f.get('severity', 'WARNING')} for f in findings if f.get('line', 0) > 0]}
