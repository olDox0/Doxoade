# doxoade/doxoade/tools/import_fixer.py
...
_IGNORED_DIRS = {'.git', 'venv', '.venv', 'env', '__pycache__', '.doxoade', '.pytest_cache', 'build', 'dist'}
...
def _detect_replacements(src: str, modules: set[str], package_depth: int, package_name: str) -> tuple[dict[ast.stmt, str], list[str]]:
    try:
        tree = ast.parse(src)
    except SyntaxError:
        return ({}, [])
...
def _best_candidate(old: str, modules: set[str]) -> str | None:
    ...
    if not cands:
        return None
    ranked = sorted(((SequenceMatcher(None, old, cand).ratio(), cand) for cand in cands), reverse=True)
