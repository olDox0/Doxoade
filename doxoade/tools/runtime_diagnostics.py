# doxoade/doxoade/tools/runtime_diagnostics.py
    ...
    def wrapped_cdll(name, *args, **kwargs):
        ...
        if isinstance(path, str) and path.lower().endswith(('.dll', '.so', '.dylib')):
            if os.path.isabs(path) and (not os.path.exists(path)):
                raise DxoadeNativeLoadError(f'DLL não encontrada: {path}\nScript: {script_path}\nVerifique se o caminho foi montado corretamente.')
        try:
            return real_cdll(name, *args, **kwargs)
        except FileNotFoundError as e:
            raise DxoadeNativeLoadError(f'Falha ao carregar DLL: {path}\nPossível causa: caminho errado ou dependência ausente.\nErro original: {e}') from e
        except OSError as e:
            raise DxoadeNativeLoadError(f'DLL encontrada, mas não carregou: {path}\nPossível causa: dependência nativa ausente (runtime C/C++).\nErro original: {e}') from e
