...
# O padrão que procuramos: 'from tools.doxcolors' no início de uma linha.
# Usamos uma expressão regular para garantir que estamos modificando apenas as linhas de importação.
# ^(\s*) -> Captura qualquer indentação inicial.
# from tools\.doxcolors\b -> Corresponde à string exata, com uma fronteira de palavra (\b) para segurança.
#BROKEN_IMPORT_PATTERN = re.compile(r"^(\s*)from tools\.doxcolors\b")
BROKEN_IMPORT_PATTERN = re.compile(r"^(\s*)from tools\.doxcolors\b", re.MULTILINE)
# A string de substituição correta.
# \1 -> Reinsere a indentação original capturada.
FIXED_IMPORT_STRING = r"\1from doxoade.tools.doxcolors"
...
def fix_imports(path, apply):
    ...
    for root, dirs, files in os.walk(project_path, topdown=True):
        # Impede a varredura dos diretórios ignorados
        ...
        for file in files:
            ...
            try:
                ...
                # Usa a regex para substituir, contando o número de substituições
